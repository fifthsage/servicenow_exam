...
[43,'What is a Discovery behavior useful for?','Coordinating multiple MID Servers, protocols, credentials, or domains in one schedule','Changing CI form labels','Creating Event Management alerts','Packaging scoped apps','Behavior는 복잡한 범위에서 프로토콜·MID Server·Credential 사용 방식을 정의합니다.'],
...
[45,'What is the main goal of classification?','Determine the device or application type and select suitable discovery logic','Choose a report color','Approve a change','Create a user account','Classification은 대상 유형을 판별하고 실행할 Probe 또는 Pattern을 결정합니다.'],
...
[50,'Where should an implementer start when a Discovery run has failures?','Discovery Status, device results, phase logs, and ECC Queue evidence','Portal homepage','User survey','Catalog request','Discovery Status에서 실패 대상과 단계를 찾고 관련 로그와 ECC 메시지를 추적합니다.'],
...
[58,'What does a partial payload mechanism help handle?','Large discovery results that must be processed without losing partial information','Portal translations','User passwords','Exam attempts','대형 Payload 처리 중 일부 정보를 잃지 않도록 분할된 IRE 처리를 지원합니다.'],
[59,'What should be checked when the same device is repeatedly inserted as new CIs?','Identifiers, source values, class selection, and IRE processing results','Dashboard layout','Email watermark','Update set preview','중복 생성은 식별 규칙과 실제 Payload 값, 클래스, IRE 결과를 함께 조사해야 합니다.'],
[60,'What defines a production-ready Discovery operation?','Governed scope, healthy MID Servers, secure credentials, monitored schedules, CMDB quality checks, and remediation ownership','One successful scan only','Unlimited IP ranges','No error monitoring','Manual CI creation after every run','범위·MID·Credential·Schedule·CMDB 품질과 오류 대응 소유권이 운영 체계로 정착되어야 합니다.'],
...
[68,'A command succeeds but a later pattern step receives an empty value. What should be inspected first?','The output variable mapping and parsing expression','The portal theme','The CI retirement rule','The exam configuration','명령 성공 후 값이 비었다면 단계 Output이 올바른 변수에 저장되었는지와 Parsing 식이 실제 출력과 맞는지 확인합니다.'],
...
[72,'What should be checked when an out-of-box pattern stops working after target software changes?','The target output format, commands, permissions, and current pattern version','Only the dashboard color','Only the Discovery schedule name','The number of portal cards','대상 버전 변경은 명령 지원, 권한과 출력 형식을 바꿀 수 있으므로 현재 Pattern 버전과 함께 비교해야 합니다.'],
...
[95,'Which schedule type is used to discover certificates from URLs?','Certificates with URL Certificate Discovery','IP addresses only','Networks','Service only','Certificates 유형은 개별 URL Scan을 통해 Certificate CI와 관련 정보를 탐색합니다.'],
[96,'What is the difference between an IP address scan and a CI scan?','An IP scan records active-address history without CMDB updates; a CI scan identifies and updates CIs','Both always create identical CIs','Only an IP scan uses Shazzam','A CI scan never uses credentials','IP Address Scan은 비인증 가시성 확인용이고 Configuration Items Scan은 Credential과 Identifier로 CMDB를 갱신합니다.'],
...
[106,'What is the purpose of the IRE Identification Simulator?','Test a payload against identification logic without committing normal production changes','Run a full subnet scan','Validate a MID Server installer','Create an incident SLA','Identification Simulator는 구성한 Payload가 어떤 CI와 일치하고 어떤 규칙을 타는지 사전 분석합니다.'],
[107,'What can cause IRE to create a duplicate CI even when the device already exists?','Missing or inconsistent identifier values in the incoming payload or existing CI','A healthy MID Server','A valid reconciliation rule','A staggered schedule','기존·신규 데이터의 Serial Number 등 식별값이 누락되거나 형식이 다르면 동일 CI 매칭이 실패할 수 있습니다.'],
...
[116,'What does the Discovery Admin Workspace Error Framework help operators do?','Aggregate, prioritize, diagnose, and remediate recurring Discovery errors','Design catalog items','Approve change requests','Create user groups','Error Framework은 반복 오류를 유형과 영향별로 모아 원인 분석 및 개선 작업을 관리하게 합니다.'],
...
[132,'A regex works in a tester but not in Discovery. What environment difference matters?','Actual line endings, escaping, encoding, and captured input variable','The portal landing page','The exam duration','The CI health score only','실제 Pattern 입력의 Escape·Encoding·개행과 Tester Sample이 다르면 같은 Regex도 결과가 달라질 수 있습니다.'],
[133,'What should be documented for a custom pattern command?','Purpose, required privilege, expected output, timeout, and failure behavior','Only the command author name','Only the target IP','Only the answer option','명령의 권한·출력·Timeout·실패 처리를 문서화하면 보안 검토와 운영 장애 분석이 쉬워집니다.'],
...
[138,'Why can changing a shared library be higher risk than changing one extension?','Multiple patterns may execute the shared steps','It changes every user password','It disables all schedules automatically','It deletes the CMDB hierarchy','재사용 범위가 넓어 한 변경의 영향 반경이 크므로 소비 관계와 결과 차이를 먼저 분석해야 합니다.'],
...
[141,'What should happen when a required identification attribute cannot be collected?','Record a clear failure and avoid creating an ambiguously identified CI','Create a CI with a random name','Reuse the last target value','Disable IRE','필수 식별값 없이 CI를 만들면 중복·오연결 위험이 있으므로 명확히 실패 처리하고 원인을 남겨야 합니다.'],
...
[149,'What is a good rollback for a faulty pattern customization?','Deactivate or revert the extension while preserving the last known-good base behavior','Delete all discovered CIs','Remove the MID Server','Clear every credential','Customization을 분리·Version 관리하면 문제가 생겼을 때 Base 동작으로 안전하게 되돌릴 수 있습니다.'],
...
[151,'A new site has no MID Server. What should drive its placement design?','Network reachability, latency, security zones, target count, and availability needs','The portal language','The number of exam domains','The CI label length','MID 배치는 Target 접근 경로와 부하·보안 경계를 기준으로 설계해야 합니다.'],
...
[153,'When is a separate MID Server appropriate for a restricted zone?','When network policy requires an execution point inside or near that security boundary','When changing a dashboard','When creating a CI identifier','When adding a catalog item','DMZ 등 보안 경계는 허용된 최소 경로만 갖는 별도 실행 지점을 두는 것이 적합합니다.'],
[154,'What should be included in MID Server capacity planning?','Concurrent schedules, target volume, protocols, pattern cost, and growth','Only the MID hostname','Only the number of users','Only the exam duration','처리량은 Target 수뿐 아니라 Protocol 지연, Pattern 명령 수와 동시 실행에 영향을 받습니다.'],
...
[162,'Why synchronize time on MID Servers and targets?','Accurate timestamps support authentication, logs, certificates, and event correlation','It changes CI classes','It increases port count','It removes credentials','Clock 차이는 Kerberos·Certificate 검증과 ECC·Target Log 시간 비교를 어렵게 만듭니다.'],
...
[166,'Why use descriptive schedule names?','They make scope, environment, owner, and purpose visible during operations','They grant MID privileges','They replace IP ranges','They change classifiers','이름에 환경·지역·범위를 담으면 Alert와 Discovery Status에서 담당 Schedule을 빠르게 식별할 수 있습니다.'],
...
[171,'What does a rising average discovery duration suggest?','Capacity, network, credential, target, or pattern performance should be investigated','Every CI is healthier','The portal is faster','IRE rules are unnecessary','수행시간 추세는 Queue 경쟁이나 Target 응답·Pattern 변화 같은 성능 저하의 조기 신호입니다.'],
...
[175,'What should a Discovery operational dashboard trend over time?','Coverage, success rate, error categories, duration, MID health, and stale CIs','Only total user count','Only portal visits','Only exam attempts','추세 지표는 일회성 성공이 아닌 Coverage와 품질·성능 변화 및 개선 효과를 보여줍니다.'],
...
[177,'What should be done before retiring a MID Server?','Move or redesign its schedules and ranges, validate replacement capacity, then decommission','Stop it without checking assignments','Delete all credentials','Retire every CI it discovered','담당 Workload와 Network 접근을 대체 MID로 검증한 뒤 제거해야 Discovery 공백을 막을 수 있습니다.'],
...
[179,'What is a useful acceptance criterion for a new Discovery scope?','Agreed coverage and success thresholds with explainable exceptions and valid CMDB results','Zero logs of any kind','One discovered device','Unlimited credentials','Coverage·성공률·예외·CI 정확성 기준을 사전에 합의해야 Go-live 판단이 객관적입니다.'],
[180,'What should a handover package for Discovery operations include?','Architecture, scopes, schedules, credentials ownership, runbooks, dashboards, and known exceptions','Only the installer file','Only admin passwords','Only a list of CI names','운영 인수에는 구성과 책임, 장애 대응 절차 및 정상 예외가 함께 있어야 지속 가능한 관리가 가능합니다.'],
...
[184,'Why is SNMPv3 preferred where supported?','It provides stronger authentication and privacy than earlier community-string models','It requires no credentials','It disables encryption','It discovers Windows processes','SNMPv3는 사용자 기반 인증과 암호화를 지원해 Community String 방식보다 보안 통제가 강합니다.'],
[185,'A network device responds to ping but SNMP classification fails. What should be checked?','UDP reachability, SNMP version, credentials, ACLs, and device configuration','Only the CI display name','Only the MID heartbeat','Only the portal cache','Ping 성공은 SNMP 접근을 보장하지 않으므로 Protocol·ACL·Credential을 별도로 검증해야 합니다.'],
...
[188,'Why can credential affinity become misleading after account rotation?','It may point to a formerly successful credential that is no longer valid','It changes the target IP','It disables IRE','It creates a new MID Server','성공 이력은 Credential 변경 후 유효하지 않을 수 있으므로 실패 시 Affinity와 현재 Alias 연결을 함께 확인합니다.'],
...
[192,'Why can the same credential work from one MID but fail from another?','Network path, domain trust, name resolution, source ACL, or local configuration may differ','IRE changes the password','The CI class encrypts it','Schedule names affect authentication','인증은 Credential 자체뿐 아니라 요청 Source와 Network·Domain Context에 의존합니다.'],
[193,'What is the correct response to repeated account lockouts caused by Discovery?','Stop retries, identify affected schedules and credential policy, then correct the configuration','Increase scan frequency','Create more wrong passwords','Disable all target auditing','반복 시도를 먼저 멈추고 Alias·Affinity·Schedule 범위와 Lockout 정책을 분석해야 서비스 계정 영향을 제한합니다.'],
...
[196,'Why can reverse DNS results not be treated as a guaranteed unique identifier?','They may be missing, stale, shared, or change independently of the device','They are always immutable','IRE accepts only DNS','They contain credentials','DNS 이름은 운영 편의 정보이며 안정적 Serial·Cloud Object ID 등을 대신하기 어렵습니다.'],
...
[198,'Why audit use of privileged Discovery credentials?','To verify use is limited to approved targets, schedules, and operators','To make scans anonymous','To bypass rotation','To remove ownership','사용 기록은 과도한 Scope나 오용을 찾고 보안 책임과 규정 준수를 입증합니다.'],
[199,'A target certificate changes and HTTPS discovery fails. What should be reviewed?','Trust chain, hostname validation, protocol settings, and the target endpoint','Only SNMP community','Only CI reconciliation','Only the portal language','Certificate 교체는 Trust·Name·TLS 호환성에 영향을 주므로 MID에서 실제 Endpoint 연결을 검증해야 합니다.'],
...
[201,'A schedule creates no Discovery Status record. Where should investigation begin?','Schedule activation, trigger timing, execution configuration, and system logs','CI duplicate tasks','Pattern parsing only','Reconciliation priority only','Status 자체가 없으면 Target 단계 전이므로 Schedule 실행 조건과 Trigger·Platform Log부터 확인합니다.'],
...
[203,'ECC output is Processed but no expected input appears. What should be checked?','MID logs, command execution, timeout, and return-message processing','Only CMDB Health','Only the schedule name','Only the portal cache','작업 수신 후 응답이 없으므로 MID 실행·Target 응답·Timeout·Input 생성 경로를 따라가야 합니다.'],
...
[205,'Only one subnet fails while others on the same MID succeed. What should be compared?','Routing, firewall, ACL, DNS, and target protocol conditions for that subnet','MID validation only','All CMDB classes','Exam configuration','MID 공통 기능은 정상일 가능성이 높으므로 실패 Segment의 Network 차이를 비교합니다.'],
...
[207,'Discovery updates attributes but misses relationships. What should be inspected?','Relationship or connection sections, child identification, and payload results','Port scan only','MID installation only','Credential encryption only','속성 성공은 대상 접근이 됐음을 뜻하므로 Connection Logic과 양 끝 CI 식별을 집중 확인합니다.'],
[208,'A CI is discovered under the wrong class. What should be reviewed?','Classifier match, pattern selection, payload class, and reclassification behavior','Only reconciliation priority','Only schedule frequency','Only dashboard filters','초기 Class 결정 경로와 IRE Payload Class를 추적해야 잘못된 Hierarchy 매핑의 원인을 찾을 수 있습니다.'],
[209,'A CI attribute changes back and forth after different imports. What is the likely cause?','Competing data sources without appropriate reconciliation design','A failed port scan','A paused MID only','A pattern timeout only','Source 간 값 경쟁은 Attribute별 갱신 권한과 우선순위를 설계해 해결해야 합니다.'],
[210,'Why use a timeline when troubleshooting Discovery?','It correlates schedule, ECC, MID, target, pattern, and CMDB events in order','It replaces all logs','It changes CI identity','It creates credentials','시간 순서로 증거를 맞추면 원인과 후속 증상을 구분하고 최근 변경의 영향을 확인할 수 있습니다.'],
...
[213,'What should be compared when Debug succeeds but the schedule fails?','MID, user context, credentials, parameters, target, and pattern version','Only question wording','Only CI age','Only portal session','두 실행의 Context 차이를 표로 비교하면 숨은 환경 차이를 빠르게 좁힐 수 있습니다.'],
[214,'A schedule discovers fewer devices after a network change. What evidence is most useful?','Before-and-after active IPs, routes, firewall rules, exclusions, and error dispositions','Only total CMDB count','Only user logins','Only pattern comments','Coverage 변화와 Network 변경을 Target 수준으로 비교해야 실제 감소와 접근 실패를 구분할 수 있습니다.'],
[215,'What does an intermittent timeout across random targets often require?','Capacity, concurrency, network latency, and target throttling analysis','A new CI identifier only','A portal restart','A catalog approval','무작위 Timeout은 단일 Logic 오류보다 공유 자원 포화나 Rate Limit 가능성을 측정해야 합니다.'],
...
[217,'What is a strong closure criterion for a recurring Discovery error?','Root cause fixed, representative reruns successful, CMDB effects checked, and monitoring confirms stability','One error record was closed manually','The dashboard was hidden','The schedule was renamed','오류 상태만 닫는 것이 아니라 원인·재시험·데이터 영향과 일정 기간 재발 여부를 확인해야 합니다.'],
...
[220,'What should a post-incident review of a Discovery outage produce?','Root cause, impact, timeline, corrective actions, owners, and prevention metrics','Only a screenshot','Only a new schedule name','Only deleted logs','기술 원인뿐 아니라 Detection·대응·재발 방지 Action과 측정 기준까지 남겨야 운영 성숙도가 높아집니다.'],
[221,'What does a de-duplication task represent in CMDB governance?','Potential duplicate CIs that require controlled review and remediation','A MID Server upgrade','A Discovery schedule retry','A credential rotation','De-duplication Task는 IRE나 Health가 찾은 중복 후보를 합병·정리하는 통제된 작업 단위입니다.'],
[222,'Why should duplicate CIs be remediated carefully rather than simply deleted?','Relationships, tasks, history, and authoritative identifiers may need preservation or reassignment','Duplicates never have relationships','Deletion improves every payload','IRE requires manual deletion','중복 CI에 연결된 운영 Record와 관계를 보존하며 올바른 CI로 통합해야 데이터 손실을 막습니다.'],
...
[227,'What context is often required with a cloud resource ID to ensure uniqueness?','Cloud provider account, subscription or project, and region or scope','Portal user language','MID Java memory only','Incident priority','Provider별 ID Namespace가 다르므로 Account·Project·Region 등 식별 Context를 함께 모델링해야 합니다.'],
[228,'Why can ephemeral cloud resources challenge traditional stale-CI rules?','Short-lived resources are expected, so lifecycle and retirement timing must reflect cloud behavior','Cloud resources never change','They cannot be CIs','IRE ignores timestamps','자동 생성·삭제가 잦은 Resource는 On-premise와 다른 갱신·Retirement 기준이 필요합니다.'],
[229,'What is event-driven cloud discovery useful for?','Updating CMDB closer to resource lifecycle changes instead of waiting only for periodic scans','Replacing all cloud credentials','Discovering only offline routers','Disabling IRE','Cloud Event에 반응하면 생성·변경·삭제를 주기 Scan보다 빠르게 반영할 수 있습니다.'],
...
[233,'What should be monitored after enabling a new cloud discovery source?','Coverage, API errors, throttling, duplicates, class mapping, and reconciliation outcomes','Only portal visits','Only MID heartbeat','Only exam attempts','Cloud API 성공뿐 아니라 CMDB Class·중복·Source 충돌까지 함께 확인해야 데이터 품질을 검증할 수 있습니다.'],
...
[236,'What is a practical way to validate cloud discovery completeness?','Compare CMDB counts and samples with provider inventory by account, region, and resource type','Assume a successful status means complete','Check one random CI only','Disable API pagination','Provider Inventory를 기준으로 Scope별 Count와 Sample 속성·관계를 대조해야 누락을 찾을 수 있습니다.'],
...
[240,'What demonstrates readiness to operate Discovery at enterprise scale?','Repeatable governance, secure access, resilient MIDs, measured coverage, trustworthy CMDB data, and owned remediation','A single successful Quick Discovery','One unrestricted administrator credential','No documented exceptions','대규모 운영은 기술 설치보다 Scope·보안·복원력·품질 지표와 오류 개선 책임이 반복 가능하게 작동하는지가 핵심입니다.']
...
(function(qs){
  function lesson(q){
    ...
    if((id>=21&&id<=27)||(id>=101&&id<=116)||(id>=221&&id<=240))return ['CMDB & IRE','실무에서는 Identifier 값과 대상 Class, Data Source, IRE 처리 결과, 관계 및 CMDB Health 지표를 한 흐름으로 검증합니다.','IRE, Identifier, Reconciliation, CMDB Health, Data Source'];
    if(id>=117&&id<=120)return ['Cloud Discovery','Provider Inventory와 CMDB를 Account·Region·Resource Type별로 대조하고, API 오류·중복·관계 누락을 함께 확인합니다.','Cloud Discovery, Service Graph Connector, API, Coverage'];
