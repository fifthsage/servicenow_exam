window.CISDF_QUESTIONS = [
  {
    ...
    "explanation": "**[핵심 근거]**\nBusiness Application, Information Object, Business Capability는 기업의 전략, 역량, 애플리케이션 포트폴리오를 설계하는 CSDM의 Design & Planning 도메인에 속합니다.\n\n**[오답 구분]**\nService Delivery는 실제 운영 서비스와 기술 서비스 인스턴스를 관리하는 영역입니다. Service Consumption은 사용자가 소비하는 비즈니스 서비스와 요청 경험에 초점을 둡니다. Build & Integration은 애플리케이션 개발과 배포 흐름에 더 가깝습니다.\n\n**[실무 연결]**\n이 구분은 CMDB에 운영 CI를 넣는 문제와 EA/APM 관점의 포트폴리오 데이터를 정리하는 문제를 분리할 때 중요합니다.\n\n**[Exam Keywords]**\nDesign & Planning, Business Application, Information Object, Business Capability, non-operational data"
  },
  {
    "id": 2,
    ...
    "explanation": "**[핵심 근거]**\nIntegrationHub ETL은 ServiceNow CMDB에 데이터를 안전하게 적재하기 위해 설계된 표준 가이드형 도구입니다. 스크립트 작성 없이도 데이터를 IRE(Identification and Reconciliation Engine)와 자동으로 연동시켜 중복 생성을 원천 차단합니다. 반면 Table API는 데이터베이스에 직접 쓰기 때문에 IRE를 우회하며, 레거시 Transform Maps는 IRE 연동을 위해 복잡한 수동 스크립팅이 필요하므로 실수로 인한 우회 위험이 존재합니다.\n\n**[Exam Keywords]**\nIntegrationHub ETL"
  },
  {
    "id": 3,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow의 CMDB Data Foundations 플레이북은 특정 CMDB 건전성 지표가 기준 미달일 때 두 가지 핵심 가치를 제공합니다. 첫째, 해당 데이터 결함이 IT 서비스 관리(ITSM)나 자산 관리 등 후속 프로세스에 미치는 부정적 영향(Downstream impacts)을 설명해 줍니다. 둘째, 관리자가 이를 올바르게 수정할 수 있도록 구체적인 조치 방법(Remediation options)을 단계별로 안내합니다. 플레이북은 자동 치유 스크립트를 실행하거나 커스텀 지표를 설계하는 도구가 아닌, 데이터 거버넌스 가이드를 제공하는 도구입니다.\n\n**[Exam Keywords]**\nCMDB, Data Foundations Dashboard, ITSM"
  },
  {
    "id": 4,
    ...
    "explanation": "**[핵심 근거]**\nCMDB 및 CSDM 데이터의 표준 준수 여부를 모니터링하는 'Data Foundations Dashboard'는 ServiceNow Store에서 무료로 다운로드하여 설치할 수 있는 정식 애플리케이션입니다. 패밀리 릴리즈 주기에 종속되지 않고 신속한 기능 업데이트를 제공하기 위해 스토어 앱 형태로 배포되며, 플랫폼의 데이터 신뢰도를 높이기 위한 공익적 목적의 툴이므로 추가 라이선스 비용이 발생하지 않습니다.\n\n**[Exam Keywords]**\nCMDB, CSDM, Data Foundations Dashboard"
  },
  {
    "id": 5,
    ...
    "explanation": "**[핵심 근거]**\n'가장 많이 보고된 값(Most Reported)' 또는 '가장 최근에 입력된 값(Last Reported)'과 같이 조건에 따라 데이터를 채택하는 동적 조정 규칙(Dynamic Reconciliation Rules)을 사용하려면, 인스턴스에 CMDB 360(기존의 Multisource CMDB) 기능이 활성화되어 있어야 합니다. CMDB 360이 켜져야 탈락한 소스의 원본 데이터까지 백엔드 데이터베이스(cmdb_multisource_data)에 보관되므로, 이를 기반으로 한 동적 비교 연산이 가능해집니다.\n\n**[Exam Keywords]**\nCMDB 360/Multisource CMDB"
  },
  {
    "id": 6,
    ...
    "explanation": "**[핵심 근거]**\n(Many-to-many). 엔터프라이즈 IT 환경에서 애플리케이션 서버와 애플리케이션 간의 관계는 다대다(Many-to-many) 구조를 가집니다. 하나의 서버가 여러 앱을 구동할 수 있을 뿐만 아니라, 하나의 대형 애플리케이션이 가용성과 부하 분산을 위해 여러 대의 서버에 걸쳐 분산 배치(Clustering/Load Balancing)될 수 있기 때문입니다. ServiceNow는 이를 지원하기 위해 두 테이블 간의 관계를 참조 필드가 아닌 독립된 관계 테이블(cmdb_rel_ci)을 활용한 Many-to-many 모델로 관리합니다.\n\n**[Exam Keywords]**\nMany-to-many"
  },
  {
    "id": 7,
    ...
    "explanation": "**[핵심 근거]**\n지문에서 '각 서버가 여러 애플리케이션을 호스팅할 수 있고, 각 애플리케이션 또한 여러 서버에서 실행될 수 있다'고 명시했으므로 두 구성요소(CI) 간의 관계는 다대다(Many-to-many)입니다. ServiceNow CMDB는 엔터프라이즈의 로드 밸런싱 및 클러스터 환경을 반영하기 위해, 기본 참조 필드 대신 'cmdb_rel_ci' 테이블을 중간 교차 테이블로 활용하여 이러한 다대다 관계를 유연하게 지원합니다.\n\n**[Exam Keywords]**\nMany-to-many"
  },
  {
    "id": 8,
  ...
  },
  {
    ...
    "explanation": "**[핵심 근거]**\nCMDB Health Dashboard의 3대 핵심 KPI(3C) 중 하나인 'Correctness(정확성)' 스코어카드는 데이터의 무결성과 최신 상태를 측정합니다. Correctness 영역은 중복(Duplicate), 고아(Orphan), 그리고 특정 기간 동안 업데이트가 발생하지 않아 방치된 좀비 데이터인 '오래된 CI(Stale CIs)'를 추적하여 관리자에게 시각화해 줍니다. 필수 필드 입력 여부는 Completeness, 규정 및 감사 준수 여부는 Compliance에서 다룹니다.\n\n**[Exam Keywords]**\nCorrectness"
  },
  {
    "id": 11,
    ...
    "explanation": "**[핵심 근거]**\nIncident 폼에서 장애 대상이 되는 구성요소(CI)를 선택하면, ServiceNow는 해당 CI 레코드에 사전에 정의된 'Support Group(지원 그룹)' 정보를 읽어와 Incident 폼의 'Assignment Group(담당 그룹)' 필드에 자동으로 입력합니다. 이는 장애 티켓을 올바른 기술 지원 팀으로 지연 없이 실시간 라우팅하여 장애 처리 프로세스를 가속화하기 위한 OOTB(기본 제공) 연동 기능입니다.\n\n**[Exam Keywords]**\nAssignment Group"
  },
  {
    "id": 12,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CI Class Manager의 Attributes(속성) 화면에서는 상속 구조를 효율적으로 파악할 수 있도록 탭 분할 뷰를 제공합니다. 그중 'Added' 탭은 상위 부모 클래스로부터 상속받은 속성(Derived 탭에 표시됨)을 제외하고, 오직 현재 클래스(Windows Server) 관리를 위해 직접(Specifically) 추가로 생성된 고유 속성만 필터링하여 표시합니다.\n\n**[Exam Keywords]**\nAdded"
  },
  {
    "id": 13,
  ...
  },
  {
    ...
    "explanation": "**[핵심 근거]**\nServiceNow 공식 문서에 따르면 CMDB 360(Multisource CMDB) 환경에서 사용할 수 있는 동적 조정 규칙 유형(Dynamic Rule Types)은 'First Reported', 'Most Reported', 'Last Reported', 'Largest Value', 'Smallest Value' 총 5가지입니다. 따라서 제시된 보기 중에서는 'Most Reported'와 'Smallest Value'만이 올바른 정답입니다. 'Last Updated'는 얼핏 정답처럼 보이지만 공식 명칭인 'Last Reported'가 아니며, 'Last Created' 역시 존재하지 않는 가짜 옵션입니다.\n\n**[Exam Keywords]**\nMost Reported, Smallest Value"
  },
  {
    "id": 16,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow의 최신 베스트 프랙티스에 따르면, CMDB 내 중복 구성요소(CI)를 정제하고 관리하는 가장 선호되는 방식은 'CMDB Workspace의 De-duplication Dashboard'를 활용하는 것입니다. 이 대시보드는 중복 현황에 대한 전사적 통계를 시각화해 줄 뿐만 아니라, 안전한 데이터 병합 및 관계 이전을 지원하는 가이드형 '치유 마법사(Remediation Wizard)' 환경을 다이렉트로 제공합니다. 클래식 메뉴의 De-duplication Task module은 단순 레코드 목록만 나열하므로 최적의 관리 방식으로 권장되지 않습니다.\n\n**[Exam Keywords]**\nCMDB, CMDB Workspace, CI"
  },
  {
    "id": 17,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB에서 새로운 커스텀 CI 클래스를 생성하는 것은 데이터베이스 테이블을 확장(Extend)하고 데이터 사전 사전을 수정하는 고도의 작업입니다. 이를 수행하려면 CMDB 관리 도구에 접근할 수 있는 'sn_cmdb_admin' 역할과, 실제 데이터베이스 필드 및 속성을 정의할 수 있는 데이터 사전 변경 권한인 'personalize_dictionary' 역할이 모두 필요합니다. 단순히 화면 레이아웃과 필드 가시성만 변경하는 'personalize_form' 권한으로는 테이블 스키마를 생성할 수 없습니다.\n\n**[Exam Keywords]**\nsn_cmdb_admin and personalize_dictionary"
  },
  {
    "id": 18,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB에서 관계 유형은 '부모-자식' 방향에 맞춰 정방향 문구(Forward Phrase)와 역방향 문구(Inverse Phrase)를 '::' 기호로 연결해 정의합니다. 상위 소프트웨어인 Application은 부모, 하위 인프라인 Server는 자식이 되므로 'Application [Runs on] Server'가 정방향, 'Server [Runs] Application'이 역방향이 되는 조합이 표준 명명 규칙에 부합합니다.\n\n**[Exam Keywords]**\nApplication > Runs on :: Runs > Server"
  },
  {
    "id": 19,
    ...
    "explanation": "**[핵심 근거]**\nCMDB Data Manager 정책은 구성요소(CI)의 수명 주기(은퇴, 아카이브, 삭제 등) 조치 전, 해당 자산의 관리 책임자에게 승인 및 확인 태스크를 발송합니다. 이때 CI Class Manager에서 정의한 클래스별 소유권 및 데이터 거버넌스 책임 그룹을 다이렉트로 활용하기 위해 매핑해야 하는 필드는 'Managed By Group'입니다. Support Group은 장애 조치용, Change Group은 변경 수행용 필드이므로 자산 수명 주기 거버넌스를 담당하는 이 문제의 목적에는 부합하지 않습니다.\n\n**[Exam Keywords]**\nManaged By Group"
  },
  {
    "id": 20,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB Data Manager의 핵심 목적은 기업의 데이터 보존 정책(Retention Policies)에 기반하여 오래되거나 불필요해진 CI 레코드의 아카이브(격리 보관) 및 영구 삭제 프로세스를 안전하게 자동화하는 것입니다. 이를 통해 CMDB의 전체 데이터 볼륨을 최적의 상태로 유지하고 시스템 성능 저하를 방지합니다. 관계 규칙 강제는 IRE 및 CI Class Manager의 영역이며, 레코드 암호화는 플랫폼 전용 암호화 모듈의 책임이므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB, IRE, CMDB Data Manager, CI Class Manager, CI"
  },
  {
    "id": 21,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow의 CMDB Data Foundations 대시보드에서 제공하는 'Services Have Owners Identified' 플레이북은 누락된 서비스 소유자 정보를 식별하고 채워 넣는 데 초점을 맞춘 복구 플레이북입니다. 실제 시험에서는 이 플레이북에서 활용되는 핵심 복구 플레이(Remediation plays)로 'Fix Data'와 'Analyze Data' 두 가지를 고르는 형태로 출제됩니다. 'Govern Data' 역시 거버넌스 단계에 포함되나, 직접적인 복구 활동을 지칭할 때는 통상 'Analyze Data'와 'Fix Data' 두 가지가 정답으로 취급됩니다.\n\n**[Exam Keywords]**\nFix Data, Analyze Data"
  },
  {
    "id": 22,
    ...
    "explanation": "**[핵심 근거]**\n기존의 레거시 상태 필드(Install Status, Operational Status 등)에서 새로운 CSDM 라이프사이클 표준 필드로 전환할 때, 기존 데이터 값들을 표준 2단계 구조에 부합하도록 매핑 규칙을 설정해야 합니다. 이 변환 가이드라인 규칙을 정의하고 수정하는 테이블은 'Life Cycle Mapping (life_cycle_mapping)' 테이블입니다. 'Life Cycle Stage Status' 테이블은 플랫폼 표준 마스터 데이터 자체를 담고 있는 사전 테이블이므로 전환 프로세스 중에 직접 수정하지 않습니다.\n\n**[Exam Keywords]**\nLife Cycle Mapping (life_cycle_mapping)"
  },
  {
    "id": 23,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow의 CMDB Data Foundations Dashboard는 플랫폼 모범 사례 준수 여부를 모니터링하고 추세를 추적하는 데 목적이 있습니다. 점수 지표와 함께 연동된 플레이북을 통해 데이터 완전성과 품질을 향상시킬 수 있는 구체적이고 실행 가능한 인사이트를 제공합니다. 반면, 중복 레코드를 탐지하고 제거하는 것은 CMDB Health Dashboard(Correctness)의 역할이며, 변경 승인 자동화는 ITSM 프로세스 워크플로우 엔진의 책임이므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB, Data Foundations Dashboard, CMDB Health Dashboard, Correctness, Completeness, ITSM"
  },
  {
    "id": 24,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow 공식 문서에 따르면, 다중 소스 데이터 분석을 위한 CMDB 360 쿼리를 생성하고 재사용하기 위해 저장(Save)하는 공간은 CMDB Workspace 내부의 'CMDB 360 탭' 아래에 위치한 'Saved queries' 카드(윈도우)입니다. CMDB Query Builder는 구성요소 간의 관계 지도를 쿼리하는 별도의 프레임워크이며, Coverage window는 소스별 데이터 점유율 통계 화면이므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB, CMDB 360, Multisource CMDB, CMDB Workspace, Query Builder"
  },
  {
    "id": 25,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB에서 새로운 자산 클래스(예: IoT 센서)를 확장할 때의 베스트 프랙티스는 2단계로 진행됩니다. 먼저 ServiceNow Store에서 'CMDB CI Class Models' 앱을 최신 버전으로 설치/업데이트하여 본사가 기본 제공하는 표준 모델 중 해당 클래스가 이미 존재하지 않는지 먼저 검증해야 합니다. 만약 존재하지 않아 새로 생성해야 한다면, 공통 속성을 상속받아 데이터 무결성을 지킬 수 있도록 적절한 부모 클래스 하위에 자식 클래스로 추가해야 합니다. 기존 스키마를 수정하거나 임의로 삭제 후 대체하는 행위는 플랫폼 무결성을 파괴하므로 금지됩니다.\n\n**[Exam Keywords]**\nCMDB, CI"
  },
  {
    "id": 26,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB에서 여러 외부 데이터 소스가 동일한 클래스 및 속성을 동시에 업데이트하려고 할 때, 특정 소스에 독점적인 데이터 신뢰 권한(Authoritative Source of Truth)을 부여하는 표준 방법은 '조정 규칙(Reconciliation Rules)'을 설정하고 데이터 소스별로 우선순위(Priority) 숫자를 할당하는 것입니다. 높은 우선순위(낮은 숫자)를 가진 소스의 데이터만 최종 반영되며 하위 소스의 데이터 오염을 차단합니다. 식별 규칙은 중복 방지용 고유 키 매칭 규칙이며, 데이터 새로고침 규칙은 데이터 최신성 감시 도구이므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB, Reconciliation Rule, Reconciliation Rules"
  },
  {
    "id": 27,
    ...
    "explanation": "**[핵심 근거]**\n레거시 Import Set 및 Transform Map 환경에서 CMDB 데이터의 중복을 방지하려면 데이터가 대상 테이블에 삽입되기 직전에 실행되는 'onBefore' 변환 이벤트 스크립트를 사용해야 합니다. 개발자는 이 단계에서 'CMDBTransformUtil' 스크립트 오브젝트를 호출하여 데이터를 IRE(식별 및 조정 엔진)로 우회 처리하고, 'ignore = true;' 설정을 통해 기본 변환 엔진의 중복 삽입 시도를 차단하는 방식을 취합니다.\n\n**[Exam Keywords]**\nonBefore"
  },
  {
    "id": 28,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB에서 방대한 데이터 볼륨을 효율적으로 제어하고 시스템 성능을 최적화하는 표준 메커니즘은 '아카이브 정책(Archive Policies)'입니다. 아카이브 정책은 보존 기간을 초과하거나 비활성화된 레코드를 메인 테이블에서 독립된 아카이브용 테이블로 안전하게 격리하여, 참조 무결성을 깨뜨리지 않고 데이터 볼륨을 경량화합니다. 커스텀 스크립트 기반의 Scheduled Jobs나 Business Rules는 데이터 유실 및 고아 관계 양산 위험이 있어 권장되지 않습니다.\n\n**[Exam Keywords]**\nArchive Policies"
  },
  {
    "id": 29,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow의 차세대 시각화 도구인 Unified Map(통합 맵)은 Next Experience 환경의 핵심 허브인 'CMDB Workspace' 내부에서 제공되는 컴포넌트 기능입니다. 관리자 및 운영자는 CMDB Workspace에서 CI 레코드를 조회할 때 해당 맵을 통해 직관적인 인프라 의존성 구조와 연동된 ITSM/ITOM 이벤트를 실시간 가시화할 수 있습니다. 수명 주기를 제어하는 Data Manager나 스키마를 설계하는 CI Class Manager에서는 이 맵에 접근할 수 없습니다.\n\n**[Exam Keywords]**\nCMDB Workspace"
  },
  {
    "id": 30,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow Unified Map의 필터 패널은 의존성 지도 상의 복잡한 노드와 관계선을 효율적으로 정제하기 위한 토폴로지 필터를 제공합니다. 대표적으로 특정 자산 클래스 레이어를 온/오프하는 'CI type' 필터와, 데이터의 출처별로 관계망을 제어하는 'Discovery source' 필터가 OOTB로 제공됩니다. Managed by group이나 Business criticality는 개별 레코드의 양식 속성이며 지도 전용 필터 패널의 기본 옵션이 아니므로 오답입니다.\n\n**[Exam Keywords]**\nDiscovery source, CI type"
  },
  {
    "id": 31,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB Data Manager의 'Certification(인증)' 정책은 담당자에게 특정 CI의 속성 데이터 값들을 검증하도록 태스크를 부여합니다. 이 태스크 폼 안에서 담당자는 잘못된 속성 필드를 직접 수정(Update)하여 데이터 무결성을 실시간으로 정비할 수 있는 편집 권한을 가집니다. 반면 'Attestation' 정책은 자산의 물리적 실존 여부(Existence)만 증명/확인하는 워크플로우를 생성하므로 세부 필드 수정 목적에는 부합하지 않습니다.\n\n**[Exam Keywords]**\nCertification"
  },
  {
    "id": 32,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB Workspace의 'My Work' 탭은 개별 관리자 및 클래스 소유자에게 할당된 실시간 자산 거버넌스 작업을 모아주는 인터페이스입니다. 이 작업 큐에 올라오는 증명(Attestation), 인증(Certification) 등 수명 주기 기반의 검증 태스크들을 뒤에서 자동으로 설계하고 생성해 내는 핵심 관리 엔진은 'CMDB Data Manager'입니다. De-duplication templates는 존재하지 않는 가짜 옵션이며, Health Dashboard는 자산 건강 상태 지표를 다루는 화면이므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB Data Manager"
  },
  {
    "id": 33,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow 공식 'Percent of Unique Locations' 플레이북(KB0832200)에 따르면, 중복 명칭을 가진 위치(Location) 레코드를 정제할 때는 데이터 무결성을 위해 순차적 프로세스를 밟아야 합니다. 먼저 중복 대상을 리뷰한 뒤, 가짜 위치 레코드를 가리키고 있던 CI 등의 하부 참조 연결고리를 진짜 표준 위치 레코드로 먼저 정정(Update)해주고, 마지막으로 연결 관계가 사라진 중복 위치 레코드를 안전하게 삭제(Delete)해야 합니다. Duplicate CI Remediator는 오직 cmdb_ci 테이블 하위 자산 전용 툴이며, cmn_location 테이블 데이터에는 적용할 수 없으므로 오답입니다.\n\n**[Exam Keywords]**\nSSO, CMDB, Data Foundations Dashboard, KB0832200, CI"
  },
  {
    "id": 34,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow는 기초 데이터(시리얼 번호 누락 등)의 품질 이슈를 체계적으로 분석(Analyze), 보정(Fix), 통제(Govern)할 수 있도록 'CMDB Data Foundations Dashboard'를 통해 공식 Get-Well Playbook(처방전 가이드)을 제공합니다. 하드웨어 시리얼 번호는 물리적 자산 속성이므로 CSDM 영역이 아니며, CMDB Health Dashboard는 자동화된 치유 태스크를 생성할 뿐 구조화된 지침서(Playbook) 자체를 제공하는 플랫폼이 아닙니다.\n\n**[Exam Keywords]**\nCMDB Data Foundations Dashboard playbooks"
  },
  {
    "id": 35,
  ...
  },
  {
    ...
    "explanation": "**[핵심 근거]**\nServiceNow의 CMDB Data Foundations Dashboard는 CMDB 아키텍처와 데이터 수집 체계가 베스트 프랙티스 규격에 맞게 운영되는지 모니터링합니다. 대표적으로 지난 90일 동안 지속적으로 갱신 활동이 일어난 활성 자산의 비율과, 데이터 적재 시 중복 방지 엔진인 IRE를 표준 가이드대로 잘 통과했는지 검증하는 지표를 제공합니다. 반면, 고아 CI 및 헬스 감사 실패 자산 리스트는 'CMDB Health Dashboard'의 고유 영역이므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB, IRE, Data Foundations Dashboard, CMDB Health Dashboard, CI"
  },
  {
    "id": 38,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB 조정 규칙 아키텍처에서 타겟 CI가 속한 가장 구체적인 하위 클래스(Child Class)의 규칙은 상위 부모 클래스(Parent Class)의 규칙보다 항상 우선하여 적용(Override)됩니다. 따라서 상위 Server 클래스의 '최솟값(Lowest Value = 50)' 규칙 대신, 하위 Windows Server 클래스의 '최빈값(Most Reported = 75)' 규칙이 최종 채택되어 데이터베이스에 75가 기록됩니다.\n\n**[Exam Keywords]**\n75"
  },
  {
    "id": 39,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB Data Manager 정책이 실행될 때 생성되는 수명주기 태스크들을 CI Class Manager에서 지정한 소유권 거버넌스와 일치시키려면 'Managed by group' 필드를 활용해야 합니다. 이 필드는 해당 CI 클래스의 데이터 관리 및 수명주기 책임을 지는 그룹을 정의하기 때문에, Data Manager의 아카이브/삭제/인증 태스크를 올바른 담당자에게 배정하는 OOTB 권장 표준 필드입니다. Support group은 장애 조치용, Change group은 인프라 변경 작업용 필드이므로 오답입니다.\n\n**[Exam Keywords]**\nManaged by group"
  },
  {
    "id": 40,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB 성공의 가장 중요한 핵심 성공 요인(CSF)은 명확한 관리 거버넌스 수립과 CMDB Health(완전성, 정확성, 준수성)의 지속적인 모니터링입니다. 데이터의 무분별한 대량 적재나 통제 없는 자유로운 수정은 데이터 오염을 가속화하는 안티 패턴입니다. 자동화 디스커버리 도구는 매우 훌륭한 자산 수집 수단이지만, 이 역시 거버넌스 체계 하에 통제되어야 가치를 발휘하므로 이 가장 궁극적인 필수 요건입니다.\n\n**[Exam Keywords]**\nEstablishing clear governance and continuously monitoring CMDB health"
  },
  {
    "id": 41,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow의 Next Experience 아키텍처는 데이터 도메인별 운영 효율성을 극대화하기 위해 허브 워크스페이스 체계를 사용합니다. Service Graph Connector의 목적은 외부 데이터를 CMDB에 적재하는 것이므로, 이를 중앙 모니터링하는 'Service Graph Connector Central' 대시보드 탭 역시 'CMDB Workspace' 내부에 통합되어 제공됩니다. 'Service Graph Connector Workspace'는 존재하지 않는 가짜 옵션이며, 'Discovery Admin Workspace'는 에이전트리스 방식의 자체 IP 스캔 디스커버리 전용 관리 공간이므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB Workspace"
  },
  {
    "id": 42,
    ...
    "explanation": "**[핵심 근거]**\nCSDM 5.0 가이드라인에 따르면, 'Design(설계)' 도메인은 Business Capability 및 Business Application과 같은 개념적 아키텍처 자산을 정의하는 영역입니다. 이 단계의 마스터 데이터를 수립하고 관리하는 핵심 페르소나는 전사 아키텍처를 설계하는 'Enterprise Architect'와 해당 애플리케이션의 비즈니스 수명 주기를 관리하는 'Application Owner'입니다. Business Relationship Manager는 Service Consumption (구 Sell/Consume) 도메인 소속이며, Service Instance (구Application Service) Owner는 Service Delivery (구 Manage Technology Management Services (구Technical Services)) 도메인 소속이므로 오답입니다.\n\n**[Exam Keywords]**\nEnterprise Architect, Application Owner"
  },
  {
    "id": 43,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow Next Experience 환경에서 CMDB 관리자가 데이터 품질(Health 지표 모니터링, 중복 CI 치유, Data Manager 정책 관리 등)을 종합적으로 제어하기 위해 사용하는 단일 허브 인터페이스는 'CMDB Workspace'입니다. 플랫폼은 파편화된 품질 관리 기능을 이 워크스페이스 내에 통합하여 제공합니다. 나머지 보기인 CMDB Audit Business Rule, Data Quality Scheduled Job, Data Quality Business Rule은 ServiceNow 표준 기능 명칭 스키마에 존재하지 않는 가짜 선지이거나 백엔드 하부 로직에 불과하므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB Workspace"
  },
  {
    "id": 44,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CI Class Manager 스튜디오 내에서 특정 클래스(Windows Server 등)를 자주 관리해야 하는 전담 소유자는 'Pinned Classes' 기능을 사용하여 해당 클래스를 내비게이션 상단에 압정(Pin)으로 고정할 수 있습니다. 이를 통해 복잡한 하부 클래스 트리를 매번 수동으로 확장하거나 검색창에 타이핑하는 번거로움 없이 다이렉트로 접근 경로를 간소화(Streamline)할 수 있습니다. 일반 메뉴나 레코드를 즐겨찾기하는 CI Favorites는 스튜디오 전용 옵션 명칭이 아니므로 오답입니다.\n\n**[Exam Keywords]**\nPinned Classes"
  },
  {
    "id": 45,
  ...
  },
  {
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB에서 데이터 중복을 방지하는 IRE(식별 및 조정 엔진)의 '식별 규칙(Identification Rules)'은 클래스 계층 구조별로 귀속되는 핵심 데이터 모델 설정입니다. 따라서 이 규칙을 생성하고 편집하는 표준 장소는 백엔드 설계 스튜디오인 'CMDB CI Class Manager'입니다. 일상적인 수명 주기 및 품질 작업을 수행하는 CMDB Workspace, 사후 건강도를 리포팅하는 Health Dashboard, 자산의 폐기 정책을 다루는 Data Manager는 규칙 설계 창구가 아니므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB CI Class Manager"
  },
  {
    "id": 47,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB Data Manager의 삭제(Delete) 및 아카이브(Archive) 정책은 데이터 유실 사고를 방지하기 위해 운영이 공식 종료된 'End of Life - Retired' 상태의 자산들을 핵심 타겟으로 삼는 것이 표준 베스트 프랙티스입니다. 일반적으로 미사용 자산을 Retire 정책으로 먼저 은퇴 상태로 전환시킨 후, 안전성이 확보된 이 'End of Life - Retired' 풀을 대상으로 삭제와 아카이브를 수행하여 무결성을 유지합니다. 가용 자산인 Inventory나 무차별적인 상태 지정은 거버넌스 규칙 위반이므로 오답입니다.\n\n**[Exam Keywords]**\nEnd of Life - Retired"
  },
  {
    "id": 48,
  ...
  },
  {
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CSDM 5.0 가이드라인에 따르면, 실제 운영 환경에 배포된 하부 인프라 자산(Infrastructure CIs), 가동 중인 소프트웨어 인스턴스(Service Instances (구 Application Services)), 그리고 이를 유지보수하는 기술 지원 단위(Technology Management Offering (구 Technical Service Offering)s) 간의 유기적인 의존 관계 체계를 정의하고 제어하는 영역은 'Service Delivery (구 Manage Technology Management Services (구Technical Services))(과거 명칭: Service Delivery)' 도메인입니다. 이 도메인은 실시간 장애 영향도 분석 및 헬프데스크 라우팅의 기준이 되는 핵심 운영계를 담당합니다. 아키텍처 설계를 다루는 Design이나 서비스 소비를 다루는 Sell or Consume 도메인은 부합하지 않으므로 오답입니다.\n\n**[Exam Keywords]**\nService Delivery (구 Manage Technology Management Services (구Technical Services))"
  },
  {
    "id": 50,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow 변경 관리 프로세스에서 Dynamic CI Group(동적 CI 그룹)을 활용해 대규모 인프라 자산에 변경 작업을 일괄 적용하고자 할 때, 해당 그룹 레코드는 Change Request 폼의 'Configuration Item' 필드에 입력되어야 합니다. 이를 통해 플랫폼 엔진이 동적 그룹 내의 CMDB 쿼리를 백엔드에서 실행하여 영향을 받는 실제 개별 자산들을 'Affected CIs' 관련 리스트에 자동으로 확장 및 매핑해 줍니다. 상위 영향도 측정용인 Business Service나 Service Offering 필드는 일괄 자산 매핑을 수행하지 않으므로 오답입니다.\n\n**[Exam Keywords]**\nConfiguration Item"
  },
  {
    "id": 51,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB Data Manager에서 라이브 클래스 테이블로부터 데이터를 제거하여 운영 가시성에서 격리시키면서도, 지정된 보존 기간 내에 안전하게 원래대로 복구(Restore)할 수 있는 가드레일을 제공하는 정책은 'Archive' 정책입니다. 아카이브 정책은 데이터를 'ar_'로 시작하는 쉐도우 테이블로 이주시켜 보존합니다. 반면 Delete는 복구가 불가능한 영구 파괴를 수행하며, Retire는 동일 테이블 내에서 상태 필드만 바꿀 뿐 레코드가 클래스에서 제거되지 않으므로 오답입니다.\n\n**[Exam Keywords]**\nArchive"
  },
  {
    "id": 52,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB 중복 치유 마법사(Duplicate CI Remediator)에서 마스터가 될 주요 자산(Main CI)을 선정할 때는 데이터 손실과 참조 단절을 방지하는 지표를 기준으로 삼아야 합니다. 가장 오래전에 생성된 자산을 선택하면 과거 누적된 ITSM 티켓(인시던트/변경 등)의 참조 무결성을 유지할 수 있으며, 관계선이 가장 많이 연결된 자산을 선택하면 서비스 매핑 토폴로지의 유실 리스크를 최소화할 수 있으므로 두 보기가 참입니다. 최근 생성 자산이나 관계가 적은 자산을 메인으로 삼는 것은 데이터 거버넌스 안티 패턴입니다.\n\n**[Exam Keywords]**\nOldest Created, Most Related Items"
  },
  {
    "id": 53,
  ...
  },
  {
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB 거버넌스에서 Attestation 정책은 자산 소유자에게 해당 자산(CI)이 현실에 여전히 실재하고 유효하다는 '존재 여부(Acknowledgement)'를 인증받는 신속한 수명주기 절차입니다. 반면, Data Certification은 감사 목적에 맞게 해당 자산 레코드 내의 IP 주소, 담당 그룹 등 '특정 세부 속성(Specific Attributes)' 필드 값들이 100% 정확한지 전수 검증하고 수정하는 정밀 데이터 감사 도구입니다. 따라서 두 도구의 목적 차이를 올바르게 설명한 이 참입니다.\n\n**[Exam Keywords]**\nCMDB, IRE, Certification, Attestation, CI, IP"
  },
  {
    "id": 55,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow에서 단순 개별 자산 레코드의 값 오류를 넘어, CSDM(Common Service Data Model) 표준 아키텍처 및 성숙도 단계(Foundation, Design, Walk, Run 등)에 부합하는 데이터 모델 정합성을 진단하고 개선하려면 'CSDM Data Foundations Dashboard'를 사용해야 합니다. 3C 품질을 다루는 CMDB Health Dashboard는 자산 레코드 자체의 무결성에 집중하며, Instance Scan은 플랫폼 커스텀 스크립트와 시스템 성능을 스캔하는 도구이므로 CSDM 정렬 품질 개선이라는 목적에 부합하지 않습니다.\n\n**[Exam Keywords]**\nCMDB, CSDM, Data Foundations Dashboard, CMDB Health Dashboard"
  },
  {
    "id": 56,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CSDM 5.0 프레임워크에서 기술적 장애를 비즈니스 및 사용자 영향도로 번역하는 가장 직접적인 매개체는 'Sell or Consume(판매/소비)' 도메인의 'Service Offering'입니다. 서비스 오퍼링은 특정 부서(Department)나 물리적 위치(Location) 단위로 구독자(Subscribers)를 매핑하여 관리하기 때문에, 시스템 장애 시 어떤 현업 사용자 집단이 타격을 입었는지 즉각 특정할 수 있습니다. 장애가 발생한 기술 자산 자체를 나타내는 Affected CIs나 시스템 환경 정보는 영향을 받는 인간 사용자 범위를 특정해 주지 못하므로 오답입니다.\n\n**[Exam Keywords]**\nService Offerings defined by Department or Location"
  },
  {
    "id": 57,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow 아키텍처에서 자산(Asset) 데이터와 CMDB 자산(CI) 데이터 간의 자동 일관성을 유지하는 표준 메커니즘은 '실시간 양방향 비즈니스 룰(Business Rules)'입니다. CI 테이블에서 값이 바뀔 때 자산을 동기화하는 비즈니스 룰과, 자산 테이블에서 값이 바뀔 때 CI를 동기화하는 비즈니스 룰이 모두 활성화되어 있어야만 데이터 유실 없는 상시 동기화가 보장됩니다. 정기적인 일괄 배치 처리를 수행하는 Scheduled Job이나 단순 개념적 매핑은 자동 필드 동기화를 구현하지 못하므로 오답입니다.\n\n**[Exam Keywords]**\nBusiness Rule, Business Rules, CMDB, CI"
  },
  {
    "id": 58,
  ...
  },
  {
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB에서 신규 자산 모델을 추가할 때의 베스트 프랙티스는 '최신 순정 기능 확인 후 표준 확장'입니다. 먼저 ServiceNow Store의 'CMDB CI Class Models' 앱을 업데이트하여 적합한 OOTB 표준 클래스가 제공되는지 확인해야 하며, 만약 직접 구축이 불가피하다면 기존 CMDB 계층 구조의 적절한 부모 CI 테이블을 확장하여 기존 식별/조정 규칙을 상속받아야 업그레이드 호환성이 유지되고 기술 부채가 최소화됩니다. 기존 OOTB 클래스를 개조하는 행위는 업그레이드를 불가능하게 만드는 안티 패턴이며, 자산 테이블 확장은 CI 모델 설계 목적에 부합하지 않으므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB, Compliance, CI, OOTB"
  },
  {
    "id": 60,
  ...
  },
  {
    ...
    "explanation": "**[핵심 근거]**\nServiceNow Agent Client Collector(ACC)는 타겟 장치 내부에서 데이터를 수집한 뒤 MID 서버로 아웃바운드(Outbound) 푸시하는 에이전트 기반 솔루션입니다. 따라서 인바운드 포트 개방이 엄격히 금지된 고보안 구역이나, 정기적인 에이전트리스 스캔 창을 놓치고 수시로 온/오프라인을 반복하는 원격 근무자 단말의 인벤토리를 누락 없이 수집할 때 가장 효과적으로 사용됩니다. 일반 데이터센터 서버는 에이전트리스가 표준이며, 네트워크 장비는 서드파티 에이전트 설치가 불가능하므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB, IRE, ACC, MID"
  },
  {
    "id": 62,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow Vulnerability Response(취약점 대응) 및 SecOps 환경에서 기술적 자산 위험을 비즈니스 중요도로 환산하기 위해서는, 실시간 가동 중인 'Service Instance (구Application Service)'와 상위의 전략적 'Business Application' 간의 관계선이 완벽히 구축되어 있어야 합니다. 'CSDM Data Foundations Dashboard'의 'Service Instances (구 Application Services) with Business Application Relations' 플레이북은 이 핵심 관계의 누락 여부를 감시하여 보안팀에 명확한 비즈니스 중요도 컨텍스트를 제공하도록 돕습니다. 위치 정보, 모델 정보, 라이프사이클 표준화율은 실시간 서비스 위험도 우선순위 산정에 직접적인 영향을 주지 못하므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB, CSDM, IRE, Data Foundations Dashboard, Business Application, Service Instance, Application Service"
  },
  {
    "id": 63,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB의 궁극적인 비즈니스 가치(Outcomes)는 시스템 의존성 가시화를 통해 비즈니스 연속성을 수호하는 운영 복원력 강화와, 장애 MTTR 단축 및 변경 작업 위험 감소를 달성하는 ITSM 프로세스 효율화입니다. 관계선을 자동으로 유지보수하는 것은 가치를 구현하기 위한 기술적 '수단/기능(Feature)'에 불과하며, 재무 데이터 관리는 ITAM(자산 관리)의 영역이므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB, MTTR, ITSM, ITAM"
  },
  {
    "id": 64,
  ...
  },
  {
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB/CSDM Data Foundations Dashboard는 시스템 아키텍처 정합성을 유지하기 위한 특화 도구입니다. 이 앱은 인스턴스 내의 핵심 구성 데이터가 표준 규격대로 올바르게 세팅되었는지 유효성을 검증하며, 기준 미달 항목이 발견될 경우 어드민이 안전하게 데이터를 클렌징할 수 있도록 가이드형 절차서인 'Playbook'을 제공합니다. 자산 관계 모델을 깨뜨릴 위험이 있는 무조건적 자동 수정이나 커스텀 지표 설계 프레임워크는 이 대시보드의 기능 범위가 아니므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB, CSDM, Data Foundations Dashboard, CMDB / CSDM, CMDB/CSDM"
  },
  {
    "id": 66,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CSDM 5.0 프레임워크에서 Service Portfolio Management(SPM)를 활용하여 현업 및 비즈니스 소비자(Business Consumers)에게 최종 비즈니스 서비스 오퍼링을 노출하고 구독/소비하도록 통제하는 영역은 'Sell or Consume(과거 명칭: Service Consumption)' 도메인입니다. 이 도메인은 서비스 카탈로그 및 비즈니스 서비스 오퍼링 테이블을 품고 있는 최상위 현업 접점 레이어입니다. 개발을 다루는 Build, 개념 도면을 다루는 Design, IT 내부 인프라 운영을 다루는 Service Delivery (구 Manage Technology Management Services (구Technical Services))는 소비자 관점의 노출 영역이 아니므로 오답입니다.\n\n**[Exam Keywords]**\nService Consumption (구 Sell or Consume)"
  },
  {
    "id": 67,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CSDM 5.0 프레임워크에서 특정 서비스 장애가 기업 매출에 미치는 타격(Revenue Loss)의 경중을 판별하고 인시던트 우선순위를 제어하는 표준 속성은 Service Offering 테이블에 존재하는 'Business Criticality(비즈니스 중요도)'입니다. 이를 Mission Critical 또는 High 등으로 설정해 두면, 하부 자산 장애 시 티켓의 영향도와 우선순위가 자동 격상되어 신속한 복구를 유도합니다. 담당 팀을 지정하는 Assignment Group이나 단순 기술 레이어 분류 태그인 Service Classification은 비즈니스 중요도 컨텍스트를 제공하지 못하므로 오답입니다.\n\n**[Exam Keywords]**\nGroup, CSDM"
  },
  {
    "id": 68,
  ...
  },
  {
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB에 외부 데이터를 Import Set 및 트랜스폼 맵을 통해 적재할 때, 중복 자산 생성을 방지하는 모범 베스트 프랙티스는 트랜스폼 맵의 'onBefore' 스크립트에서 'CMDBTransformUtil' API를 호출하는 것입니다. 이 API는 들어오는 페이로드를 식별 및 조정 엔진(IRE)으로 리다이렉트하여 정교한 식별 규칙을 거치도록 강제합니다. 일반 테이블에서 쓰이는 Coalesce 방식은 CMDB 자산 적재 시 IRE를 완전히 우회하여 무결성을 파괴하는 안티 패턴이므로 오답입니다.\n\n**[Exam Keywords]**\nTransform Map, Import Set, Import Sets, Coalesce, CMDB, IRE, CI, API"
  },
  {
    "id": 70,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow IRE(식별 및 조정 엔진) 환경에서 자식 클래스에 오버라이드된 고유 식별 규칙이 매칭에 실패했을 때, 신규 생성을 유보하고 상위 부모 클래스의 식별 규칙(Parent Rules)을 연쇄적으로 시도하도록 강제하는 식별자 엔트리 설정 속성은 'Allow fallback to parent's rules'입니다. 이 가드레일 옵션을 활성화해야 범용적인 부모 식별 기준을 통해 무분별한 중복 자산 인서트를 막을 수 있습니다. 클래스 대상 지정 필드인 Applies to나 종속성 여부를 가르는 Independent는 폴백 로직을 제어하지 못하므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB, IRE, CI Class Manager, Identification Rule, CI"
  },
  {
    "id": 71,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow에서 노후화되거나 유용성이 상실된 자산(CI) 데이터를 탐지하여 자동으로 은퇴(Retire), 아카이브(Archive), 영구 삭제(Delete) 등 단계별 수명 주기를 제어하고 클렌징하는 표준 거버넌스 관리 도구는 'CMDB Data Manager'입니다. 스키마를 설계하는 Class Manager, 아키텍처 정렬도를 보는 Foundations Dashboard, 품질 상태를 모니터링하는 Health Dashboard는 직접적인 정책 기반 레코드 제거 프로세스를 수행하지 못하므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB Data Manager"
  },
  {
    "id": 72,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB Workspace는 통합 거버넌스 허브로서 내부에 Data Manager, Duplicate Remediator, CI Class Manager 엔진이 컴포넌트로 탑재되어 있어 외부 메뉴 이동 없이 즉시 실행이 가능합니다. 반면, 네트워크를 직접 스캔하여 데이터를 수집하는 ServiceNow Discovery 실행은 ITOM 자산 가시성(ITOM Visibility) 엔진 및 전용 Discovery 스케줄 메뉴의 고유 영역이므로 CMDB Workspace에서 직접 구동하는 액션 범주에 포함되지 않아 오답입니다.\n\n**[Exam Keywords]**\nCMDB, IRE, CMDB Data Manager, CI Class Manager, Certification, Attestation, Discovery, CMDB Workspace"
  },
  {
    "id": 73,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB Health Dashboard의 완전성(Completeness) 지표를 제어하기 위해서는 두 가지 설정이 결합되어야 합니다. 첫째, 특정 클래스(Linux Server)에서 어떤 필드가 누락되면 안 되는지 권장 필드 규칙을 지정하는 'CI Class Manager'가 필요하며, 둘째, 누락 자산 식별 시 실제로 시스템이 치유 태스크(Remediation Task)를 자동으로 발행하도록 제어하는 전역 스위치인 'Health Preferences'를 활성화해야 합니다. 단순 UI 뷰포트나 서비스 토폴로지 구성 요소는 완전성 데이터 가드레일을 설계하지 못하므로 오답입니다.\n\n**[Exam Keywords]**\nCI Class Manager and Health Preferences"
  },
  {
    "id": 74,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CSDM 5.0 성숙도 프레임워크 가이드라인에 따라, IT 내부의 소유권 및 지원 조직 체계를 뜻하는 'Technology Management Service (구 Technology Management Service (구Technical Service))'와 'Technology Management Offering (구 Technical Service Offering)'의 구축 정합성을 진단하는 영역은 'Walk(걸어가기)' 단계의 고유 지표입니다. 기초 자산을 다루는 Crawl, 대고객 비즈니스 서비스를 다루는 Run, 고급 데이터 거버넌스를 다루는 Fly 탭은 검진 대상 도메인이 다르므로 오답입니다.\n\n**[Exam Keywords]**\nWalk"
  },
  {
    "id": 75,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow IntegrationHub ETL은 외부 데이터 소스를 GUI 환경에서 변환하는 범용 데이터 파이프라인 도구입니다. 이 도구는 CMDB뿐만 아니라 일반 마스터 테이블(Non-CMDB)로의 적재를 동시에 지원하며, CMDB 적재 시 별도의 스크립트 작성 없이도 플랫폼 표준 엔진인 IRE를 강제로 경유하도록 아키텍처가 설계되어 있어 중복 데이터를 원천 차단합니다. 네트워크 스캔은 Discovery의 영역이며, SCCM 데이터 로드는 Service Graph Connector 전용 앱의 영역이므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB, IRE, IntegrationHub ETL, Service Graph Connector, Discovery, ETL, GUI, SCCM"
  },
  {
    "id": 76,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB Health Dashboard의 정확성(Correctness) 내 노후화(Staleness) 메트릭은 자산 데이터가 오랫동안 갱신되지 않았는지를 추적합니다. 이를 계산하는 표준 시스템 필드는 데이터의 수동 수정 및 자동화 수집 갱신을 모두 포괄하여 실시간 반영하는 'Updated (sys_updated_on)' 필드입니다. 최초 생성일은 데이터의 활성화 여부를 대변하지 못하며, 디스커버리 전용 필드들은 수동이나 API로 관리되는 자산에 적용할 경우 전체 정확도 점수를 오염시키기 때문에 오답입니다.\n\n**[Exam Keywords]**\nUpdated (sys_updated_on)"
  },
  {
    "id": 77,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow의 현대적인 넥스트 익스피리언스 아키텍처에서 'CMDB Data Manager'의 정책 생성, 게시, 관리 워크플로우는 'CMDB Workspace' 내부의 'Management' 탭으로 통합 관리 주소지가 일원화되었습니다. 이 탭이 자산 데이터의 전사 수명 주기 가드레일을 통제하는 핵심 센터 역할을 합니다. 데이터 소스 다각화 품질을 분석하는 CMDB 360 탭, ITSM 티켓 운영 공간인 Service Operations Workspace, 데이터 모델 스키마를 설계하는 Class Manager는 정책 기획 및 배포 인터페이스를 제공하지 않으므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB Workspace - Management tab"
  },
  {
    "id": 78,
  ...
  },
  {
    ...
    "explanation": "**[핵심 근거]**\nServiceNow IRE(식별 및 조정 엔진) 아키텍처 표준 규칙에 따라, 조정 규칙(Reconciliation Rules)은 오직 기존에 매칭된 자산 레코드를 '수정(Update)'하는 시점에만 화이트리스트 권한을 검사합니다. 시스템에 존재하지 않는 신규 자산을 최초로 생성하는 '삽입(Insert)' 단계에서는 어떠한 조정 규칙도 개입하지 않고 우회(Bypass)하므로, Altiris 소스의 신규 자산 삽입 시도는 무조건 허용(Allow Insert)됩니다. 트랜잭션의 본질을 파악하지 못하고 단일 소스 제한 정책에 함몰되어 (차단)을 선택하지 않도록 각별히 주의해야 합니다.\n\n**[Exam Keywords]**\nCMDB, IRE, Reconciliation Rule, Reconciliation Rules, Discovery, CI"
  },
  {
    "id": 83,
  ...
  },
  {
    ...
    "explanation": "**[핵심 근거]**\nServiceNow 서비스 그래프 커넥터(SGC)는 글로벌 벤더와 공동 개발하여 데이터 정합성을 보증하는 인증 제품입니다. 따라서 관리자가 기본 제공되는 OOTB 매핑 구조를 임의로 커스터마이징하면 제품 업그레이드 시 스킵 로그가 발생할 뿐만 아니라, ServiceNow 공식 기술 지원 및 보증 범위(Supportability)를 즉시 벗어나게 되므로 향후 모든 유지보수 및 디버깅 책임은 고객사(Customer)가 전적으로 부담하게 됩니다.\n\n**[Exam Keywords]**\nCMDB, IRE, Service Graph Connector, SGC, OOTB"
  },
  {
    "id": 89,
  ...
  },
  {
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB Data Manager의 데이터 필터는 텍스트 부분 일치를 판독하는 Contains 연산자를 사용합니다. 'Windows Server 2022 Datacenter'는 'Server' 문자열을 포함하고, 'Linux CentOS'는 'Linux' 문자열을 포함하므로 필터를 통과합니다. 'Windows 2019 Datacenter' 역시 데이터베이스상 부모인 Server 테이블 환경에 속한 서버용 에디션 레코드이므로 정책 영향 범주에 동시 포함됩니다. 반면 유닉스 계열인 AIX 7.2는 두 키워드를 모두 포함하지 않아 제외되므로 오답입니다.\n\n**[Exam Keywords]**\nWindows Server 2022 Datacenter, Linux CentOS, Windows 2019 Datacenter"
  },
  {
    "id": 91,
    ...
    "explanation": "**[핵심 근거]**\n상위 Server에서 더 구체적인 하위 Windows Server 클래스로 이동하는 것은 정밀 분류 방향의 Class upgrade입니다.\n\n**[Exam Keywords]**\nClass upgrade"
  },
  {
    "id": 92,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow Store 앱인 CMDB Data Foundations Dashboard는 베스트 프랙티스 기준에 맞추어 인스턴스의 데이터 품질 상태를 진단하고 점수화하여 보여주는 시각화 솔루션입니다. 따라서 거버넌스적 방향성을 수립할 수 있는 핵심 메트릭 지표를 제공하는 것이 주임무입니다. 메트릭 설정 자체는 CI Class Manager의 영역이며, 정책 태스크 처리는 CMDB Data Manager의 영역이므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB, Data Foundations Dashboard, CMDB Data Manager, CI Class Manager, CI"
  },
  {
    "id": 93,
  ...
  },
  {
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB 아키텍처에서 Health Inclusion Rules(건강도 포함 규칙)는 특정 클래스의 자산 중 건강도 진단 및 식별 프로세스의 대상이 될 레코드의 범위를 좁히는(Narrow the scope) 필터 역할을 수행합니다. 예를 들어 '가동 중(Operational)'인 자산만 솎아내어 품질 점수를 매기도록 통제합니다. 유입 데이터를 사전 차단하는 기능이나 소스 우선순위를 정하는 Reconciliation Rules와는 데이터 작동 단계가 전혀 다르므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB, IRE, CI Class Manager, Reconciliation Rule, Reconciliation Rules, Health Inclusion Rule, CI"
  },
  {
    "id": 96,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow 아키텍처 거버넌스 가이드라인에 따라, 고객이 생성하는 모든 사용자 정의(User-defined) 구성 요소에는 'u_' 접두사가 강제됩니다. 따라서 CMDB 계층 구조를 확장하여 생성하는 모든 커스텀 클래스 테이블의 물리적 명칭은 반드시 'u_cmdb_ci_' 패턴으로 시작해야만 향후 본사 업그레이드 패치 시 순정(OOTB) 테이블과의 명칭 충돌 및 그에 따른 기술 부채를 원천 차단할 수 있습니다.\n\n**[Exam Keywords]**\nu_cmdb_ci_"
  },
  {
    "id": 97,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB에서 자산 식별 가드레일을 통제하는 방법은 두 가지 경로가 있습니다. GUI 통합 환경을 제공하는 'CI Class Manager' 내의 Identification 탭을 이용하거나, 내비게이터에서 'CI Identifiers' 모듈을 검색하여 백엔드 레코드 리스트에 직접 접근하는 방식입니다. API 설정은 연동 통로일 뿐이며, 'IRE' 자체는 백엔드 구동 알고리즘의 이름일 뿐 독립된 메뉴 명칭으로 존재하지 않으므로 오답입니다.\n\n**[Exam Keywords]**\nCI Class Manager, CI Identifiers module"
  },
  {
    "id": 98,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB Workspace 아키텍처 가이드라인에 따라, 특정 자산(CI)에 걸려 있는 활성 인시던트(Incidents), 문제(Problems) 등 연계된 ITSM 프로세스 레코드들의 실시간 현황은 'Related Items' 패널을 통해 통합 가시성을 확보할 수 있습니다. 자산의 기본 스펙 요약을 보여주는 Overview, 상위 서비스 의존성을 보여주는 Service Instances (구 Application Services), 스키마 필드를 관리하는 Attributes 패널은 외부 티켓 리스트 인터페이스를 제공하지 않으므로 오답입니다.\n\n**[Exam Keywords]**\nRelated Items panel"
  },
  {
    "id": 99,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB 건강도 체계에서 지정된 필수(Required) 또는 권장(Recommended) 필드 속성 값이 채워지지 않고 누락된 자산 현황을 계측하고 관리하는 고유 스코어카드는 'Completeness(완전성)'입니다. 감사의 정책 준수 여부를 검사하는 Compliance나 중복/노후/고아 자산을 추적하는 Correctness는 속성 데이터의 누락률 자체를 산출하는 지표 구역이 아니므로 오답입니다.\n\n**[Exam Keywords]**\nCompleteness"
  },
  {
    "id": 100,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CSDM 5.0 표준 가이드라인에 따라, IT 운영 팀이 시스템 관제 및 성능 감시를 위해 내부적으로 사용하는 'Product Online Monitoring Systems'는 대표적인 기술 서비스(Technology Management Service (구 Technology Management Service (구Technical Service))) 모델에 해당합니다. 따라서 'IT를 위한 IT 관리'를 전담하는 'Service Delivery (구 Manage Technology Management Services (구Technical Services))' 도메인에 매핑하는 것이 아키텍처 표준입니다. 기초 데이터를 다루는 Foundation, 개발 단계를 다루는 Build, 현업 소비 장소인 Consumption, 개념 설계를 담당하는 Design 도메인은 모두 오답입니다.\n\n**[Exam Keywords]**\nService Delivery (구 Manage Technology Management Services (구Technical Services))"
  },
  {
    "id": 101,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB 아키텍처 가이드라인에 따라, 유입되는 자산 데이터의 고유성을 판별하여 중복을 차단하는 식별 규칙(Identification Rules) 및 엔트리는 'CI Class Manager' 내부의 'Identification Rule' 탭에서 시각적으로 설계 및 구성합니다. 소스별 데이터 수정 권한 우선순위를 지정하는 Reconciliation Rules, 테이블 기본 메타데이터를 보여주는 Basic Info, 즐겨찾기 기능인 Pinned Classes는 식별 규칙 편집 기능을 제공하지 않으므로 오답입니다.\n\n**[Exam Keywords]**\nIdentification Rule tab"
  },
  {
    "id": 102,
  ...
  },
  {
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB 쿼리 빌더에서 자산 간의 의존성 관계 토폴로지를 그리는 기본 뼈대는 'CMDB Query'로 시작해야 합니다. 여기에 자산 데이터가 아닌 ITSM 운영 데이터(Incident)를 조건 결합하려면, 좌측 팔레트에서 'Non-CMDB table' 섹션을 확장하여 Incident 테이블을 드래그한 뒤 Database 노드와 참조 링크로 매핑해주어야 합니다. Service Mapping Query는 순수 자동화 서비스 맵 탐색 전용 엔진으로 외부 트랜잭션 테이블 조인 인터페이스를 제공하지 않으므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB, IRE, Service Mapping, Query Builder, Service Instance, Application Service, ITOM, CI"
  },
  {
    "id": 108,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB Data Foundations Dashboard는 CSDM 규격에 입각하여 핵심 인프라 데이터의 유효성과 구성 정합성을 검증해 주는 주 진단 엔진입니다. 또한, 데이터의 강제 자동 변형으로 인한 참조 무결성 파괴를 방지하기 위해 임의 자동 조치를 취하는 대신, 어드민에게 안전한 표준 클렌징 절차를 안내하는 전용 'Playbooks'를 제공하여 조치를 보조하는 것이 아키텍처적 본질입니다. 커스텀 메트릭 프레임워크 빌드 기능은 포함하지 않으므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB, CSDM, Data Foundations Dashboard"
  },
  {
    "id": 109,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CSDM 5.0 거버넌스 원칙에 따라, 최종 비즈니스 사용자(Business Consumers)에게 서비스 오퍼링을 노출하여 실제 요청 및 소비를 발생시키는 단계를 관할하는 영역은 'Service Consumption (구 Sell/Consume)' 도메인이 유일합니다. 개념적 기획서와 구조를 정의하는 Design 단계나 내부 IT 인프라 운영을 제어하는 Service Delivery 단계는 소비자와의 직접적인 소비 계약 및 청구 거래가 일어나는 주소지가 아니므로 오답입니다.\n\n**[Exam Keywords]**\nService Consumption (구 Sell/Consume)"
  },
  {
    "id": 110,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CSDM 및 ITIL 모범 사례에 따라, 특정 대고객 서비스가 비즈니스 및 매출에 미치는 경제적 치명도는 'Service Offering' 레코드 내부의 'Business Criticality' 속성을 통해 정의됩니다. 인시던트 발생 시 이 속성 값을 기반으로 티켓의 Impact와 Urgency가 연산되어 최우선 순위(P1)로 수평 전파됩니다. 담당 팀을 지정하는 Assignment Group, 피해 장비를 나열하는 Affected CIs, 서비스의 기술적 종류를 구분하는 Service Classification은 우선순위 산정 메커니즘을 제공하지 않으므로 오답입니다.\n\n**[Exam Keywords]**\nGroup, CSDM, ITSM, ITIL, P1"
  },
  {
    "id": 111,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB 데이터 통합 가이드라인에 따라, 외부 플랫 파일이나 임포트 세트 데이터를 적재할 때 중복 생성을 원천 차단하고 순정 IRE의 식별/조정 규칙을 적용하는 표준 솔루션은 트랜스폼 스크립트 내에서 'CMDBTransformUtil' API를 구동하는 것입니다. 플랫폼 공통 기능인 단순 Coalesce 매칭은 CMDB 계층 구조 및 다중 소스 거버넌스 룰을 무시하여 데이터 오염을 유발하므로 금지되며, 자동 연동 프로퍼티나 비교 규칙은 시스템에 존재하지 않는 가상의 개념이므로 오답입니다.\n\n**[Exam Keywords]**\nTransform Map, Import Set, Import Sets, Coalesce, CMDB, IRE, CI, API"
  },
  {
    "id": 112,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CSDM 5.0 표준 아키텍처 가이드라인에 따라, 인프라 자산(CI)의 실질적인 운영 및 장애 배정을 제어하는 'Support Group' 필드는 해당 자산이 귀속되어 실제 운영 계약을 수행하는 'Technology Management Offering (구 Technical Service Offering)' 레코드로부터 동적으로 동기화(Dynamic Group Sync) 및 상속받는 것이 마스터 표준입니다. 연결 통로인 Dynamic CI Group이나 추상적 부모 레이어인 Technology Management Service (구 Technology Management Service (구Technical Service))는 속성의 진실의 원천이 아니며, Class Manager 설정은 범용 기본값에 불과하여 서비스 기반 동기화 대상에서 밀려나므로 오답입니다.\n\n**[Exam Keywords]**\nGroup, SSO, CMDB, CSDM, CI"
  },
  {
    "id": 113,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB 워크스페이스 아키텍처는 데이터 거버넌스의 통합 관리를 지향합니다. 따라서 라이프사이클 정책을 수립하는 Data Manager, 실시간 품질 오류를 청소하는 중복 제거 보정 마법사, 스키마 뼈대를 디자인하는 Class Manager 기능은 모두 워크스페이스 내부 탭과 타일 형태로 빌트인 되어 있어 즉각 실행이 가능합니다. 반면, 네트워크 통신 및 IP 스캔을 직접 제어하는 Discovery 실행 기능은 독립된 ITOM Discovery 관리 도메인의 주소지이므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB, IRE, CMDB Data Manager, CI Class Manager, Certification, Discovery, CMDB Workspace, IP"
  },
  {
    "id": 114,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow 데이터 통합 아키텍처 가이드라인에 따라, IntegrationHub ETL은 로코드 기반 인터페이스 내부에 순정 IRE를 기본 내장하여 데이터 오염과 중복을 자동으로 방지합니다. 또한 레거시 트랜스폼과 달리 단일 소스를 기반으로 CMDB 자산과 일반 비CMDB 테이블 모두에 다차원 동시 적재 처리가 가능한 RTE 아키텍처를 기반으로 합니다. 네트워크 탐색을 수행하는 것은 Discovery이며, SCCM 수집 전용 패키지는 Service Graph Connector이므로 오답입니다.\n\n**[Exam Keywords]**\nTransform Map, Transform Maps, CMDB, IRE, IntegrationHub ETL, Service Graph Connector, Discovery, ETL"
  },
  {
    "id": 115,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CSDM 5.0 아키텍처 가이드라인에 따라, 실제 가동 중인 인프라 자산(Infrastructure CIs)과 이들이 형성하는 실시간 서비스 인스턴스(Service Instances (구 Application Services)), 그리고 이를 지원하는 IT 내부 엔지니어 조직(Technology Management Offering (구 Technical Service Offering)s) 간의 수직적 운영 관계성을 제어하는 전담 구역은 'Service Delivery (구 Manage Technology Management Services (구Technical Services))' 도메인입니다. 개념적 기획서와 메타데이터만 정의하는 Design 단계나 대고객 판매 접점인 Service Consumption 단계는 실전 런타임 자산들의 물리적 관계선을 통제하는 주소지가 아니므로 오답입니다.\n\n**[Exam Keywords]**\nService Delivery (구 Manage Technology Management Services (구Technical Services))"
  },
  {
    "id": 116,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CSDM 5.0 및 ITIL 거버넌스 아키텍처에 따라, Technology Management Offering (구 Technical Service Offering)이 Dynamic CI Group 관계선을 통해 하위 인프라 자산들과 정렬될 때, 플랫폼의 내장 동기화 엔진(Dynamic Group Sync)은 실시간 운영계 핵심 데이터인 'Managed by Group'과 'Support Group' 필드만을 자동 전파 및 동기화합니다. 재무적 소유권을 다루는 Owned by Group이나 컴플라이언스 변경 승인을 제어하는 Approval Group은 개별 자산 또는 별도 자산 도메인의 고유 권한을 보장하기 위해 자동 동기화 파이프라인에서 제외되므로 오답입니다.\n\n**[Exam Keywords]**\nManaged by Group, Support Group"
  },
  {
    "id": 117,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB 건강도(Health) 아키텍처 가이드라인에 따라, 특정 하위 메트릭의 평가 대상 분모(모수) 규모를 조건부로 필터링하여 제한하는 전용 제어 도구는 'Health Inclusion Rule'입니다. Orphan 메트릭에만 특정 상태 값(예: Operational)을 타겟팅하는 포함 규칙이 활성화되어 있었기 때문에 분모가 50,000대로 축소되어 연산된 것입니다. Duplicate 메트릭은 규칙이 없어 전체 60,000대를 다 평가했습니다. 단순 논리 컨테이너인 CMDB Group은 메트릭별 평가 분모 자산을 동적으로 필터링 차단하는 엔진을 제공하지 않으므로 오답입니다.\n\n**[Exam Keywords]**\nGroup, CMDB, CMDB Health Dashboard, Health Inclusion Rule, Correctness"
  },
  {
    "id": 118,
  ...
  },
  {
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB 건강도(Health) 아키텍처에 따라, Compliance 스코어카드는 플랫폼의 'Desired State Audit' 엔진과 직접 연동되어 점수를 산출합니다. 특정 서버 클래스를 타겟팅하는 'Certification Filter', 통과해야 할 패치 기준을 수립하는 'Certification Template', 그리고 이를 최종 집행하는 'Audit' 레코드가 구성되어야만 준수성 대시보드가 구동됩니다. Correctness 지표인 Stale/Orphan/Duplicate나 Data Manager 속성인 은 준수성 점수 연산 파이프라인을 형성하지 않으므로 오답입니다.\n\n**[Exam Keywords]**\nCertification Filter, Certification Template, Audit configuration, and Compliance Score Calculation Job"
  },
  {
    "id": 121,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow 플랫폼 거버넌스 가이드라인에 따라, CMDB Workspace의 'My Work' 탭에 실시간 팝업되는 데이터 정제, 존재 증명(Attestation), 자산 은퇴 등의 실무 티켓들은 'CMDB Data Manager' 정책 엔진이 백엔드에서 라이프사이클 룰에 의거해 자동으로 생성하고 담당자에게 배정해 주는 것입니다. 단순 모니터링 스코어카드인 Health Dashboard나 중복 조치용 양식 툴인 De-duplication templates는 정책 기반의 대량 수명 주기 태스크 생성 파이프라인을 구동하지 못하므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB Data Manager"
  },
  {
    "id": 122,
  ...
  },
  {
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB 데이터 모델 표준에 따라, 런타임 호스트 엔진인 Application Server([cmdb_ci_app_server])는 독립된 소프트웨어 인스턴스인 Application([cmdb_ci_appl])을 하나 이상 구동하는 부모 역할을 수행합니다. 따라서 테이블 배열 순서상 '애플리케이션 서버'에서 '애플리케이션' 방향으로의 의존성 차수는 'One-to-many'(1 대 다)가 정확합니다. 논리적 서비스 계층이 아닌 물리적 프로세스 종속이므로 Many-to-many 구조는 오답입니다.\n\n**[Exam Keywords]**\nOne-to-many"
  },
  {
    "id": 124,
  ...
  },
  {
    ...
    "explanation": "**[핵심 근거]**\nServiceNow의 현대적인 CMDB 아키텍처 가이드라인에 따라, 중복 자산(Duplicate CIs)의 정제 관리를 위한 가장 권장되는 방식(Preferred Way)은 차세대 통합 UI인 'CMDB Workspace' 내부의 'De-duplication Dashboard'를 활용하는 것입니다. 이 대시보드는 자산 클래스별 중복 통계 시각화와 전용 조치 마법사(Remediation Wizard) 진입을 원스톱으로 지원합니다. 레거시 텍스트 리스트 뷰 형태인 De-duplication Task 모듈은 단순 테이블 조회를 제공할 뿐이므로 현대적 최선의 방식(Preferred Way) 맥락에서 오답입니다.\n\n**[Exam Keywords]**\nCMDB, CMDB Workspace, UI"
  },
  {
    "id": 128,
  ...
  },
  {
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB 아키텍처 베스트 프랙티스에 따라, 인시던트(Incident) 폼 같은 ITSM 트랜잭션 화면에서 검색 노이즈를 줄이고 핵심 자산만 표출하도록 필터링해 주는 'Principal Class' 속성은 'CI Class Manager' 내부의 클래스 고유 정보 정의 화면에서 체크박스를 활성화하여 제어합니다. 데이터 수명 주기를 제어하는 Data Manager나 가시성 전체 스위치만 켜고 끄는 System Properties는 특정 클래스를 주요 클래스로 커스텀 매킹하는 주소지가 아니므로 오답입니다.\n\n**[Exam Keywords]**\nCI Class Manager"
  },
  {
    "id": 130,
  ...
  },
  {
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB 아키텍처 가이드라인에 따라, CMDB Data Manager는 자산 레코드의 수명 주기 정책(Lifecycle Policies)을 자동화하기 위한 핵심 툴입니다. 은퇴하거나 방치된 자산 데이터를 보존 기간(Retention Policies)에 근거해 자동으로 아카이브하거나 삭제 처리함으로써 데이터 무결성과 인스턴스 성장을 통제합니다. CI 간의 관계 규칙을 통제하는 것은 CI Class Manager이며, 데이터 암호화는 Encryption 모듈의 스코프이므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB, IRE, CMDB Data Manager, CI Class Manager, CI"
  },
  {
    "id": 132,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow 데이터 연동 표준 거버넌스 가이드라인에 따라, 외부 데이터를 CMDB로 임포트할 때 IRE를 우회(Bypass)하는 설계적 오류를 차단하는 최선의 권장 도구는 'IntegrationHub ETL'입니다. 이 도구는 로코드 기반 마법사를 통해 백엔드에서 순정 IRE API 파이프라인 통과를 네이티브하게 강제하여 중복 생성을 예방합니다. 직접적인 Table API는 IRE를 100% 무시하고 DB에 직접 쓰기 때문에 금기시되며, 레거시 트랜스폼 맵은 수동 스크립팅 누락 시 바이패스 리스크가 존재하므로 오답입니다.\n\n**[Exam Keywords]**\nIntegrationHub ETL utilizing Robust Transform Engine (RTE) frameworks"
  },
  {
    "id": 133,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CSDM 거버넌스 명세에 따라, 구성 항목들 간의 관계 네트워크가 올바르게 적재되면 변경 관리 프로세스는 하부 자산 변동이 최상위 서비스에 미치는 피해 범위(Business Impact)를 완벽히 예측할 수 있습니다. 동시에, 각 기술 자산 레이어에 매핑된 소유권 정보를 기반으로 변경 티켓을 적절한 담당 그룹으로 동적 라우팅하는 자동화 가치가 실현됩니다. 일정 충돌 제어는 변경 모듈 고유 정책이며, 근본 원인 규명은 문제 관리(Problem)의 영역이므로 오답입니다.\n\n**[Exam Keywords]**\nGroup, CSDM, IRE, ROI"
  },
  {
    "id": 134,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CSDM 거버넌스 표준 아키텍처에 따라, 자산 수명 주기는 'Stage'와 'Stage Status'의 2단계 매트릭스로 제어됩니다. 최상위 단계인 Stage 자체는 시스템 핵심 코드로 고정되어 수정할 수 없으며, 그 하위에 종속되는 세부 선택 값 세트와 관계 규칙을 등록 및 수정할 수 있는 물리 테이블은 'Life Cycle Stage Status [life_cycle_stage_status]'가 유일합니다. 레거시 필드와의 번역 룰만 관리하는 Mapping 테이블은 상태 값 자체를 개조하는 주소지가 아니므로 오답입니다.\n\n**[Exam Keywords]**\nLife Cycle Stage Status ([life_cycle_stage_status])"
  },
  {
    "id": 135,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow 데이터 거버넌스 가이드라인에 따라, CMDB Data Foundations Dashboard는 CSDM 기준에 부합하는 데이터 모델 품질 상태를 시계열 트렌드로 추적하여 변화 과정을 모니터링할 수 있도록 돕습니다. 동시에 문제 해결을 위한 단계별 처방전인 'Remediation Playbook'을 내장하여 실행 가능한 조치 인사이트를 제공합니다. 중복 제거 제어는 De-duplication Dashboard의 고유 권한이며, 변경 승인 자동화는 변경 관리 엔진의 영역이므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB, CSDM, IRE, Data Foundations Dashboard, Completeness"
  },
  {
    "id": 136,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow 데이터 컴플라이언스 가이드라인에 따라, CMDB Data Foundations Dashboard는 중요 데이터 아키텍처가 적절하게 설정되어 있는지 유효성을 검증하는 진단 능력이 뛰어납니다. 또한 데이터 오류를 해결할 수 있도록 지원하는 단계별 안내서인 'Remediation Playbook'을 제공하여 수동 조치 프로세스를 보조합니다. 자산의 결함 데이터를 사람이 검토하기도 전에 독단적으로 강제 자동 수정하는 완전 자동화 기능이나 커스텀 지표 확장 프레임워크는 시스템의 설계 스코프를 벗어난 오답입니다.\n\n**[Exam Keywords]**\nCMDB, CSDM, Data Foundations Dashboard, Compliance"
  },
  {
    "id": 137,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CSDM 표준 데이터 모델 아키텍처에 따라, 정보 객체(Information Object)는 비즈니스 애플리케이션(Business Application)이 취급하고 처리하는 민감/규제 데이터의 '논리적 범주(Logical Data Taxonomy)'를 명시하는 메타데이터 자산입니다. 이를 결합함으로써 특정 앱의 보안 위험도와 컴플라이언스 준수 현황을 시각화할 수 있습니다. 일반 하드웨어 CI 성능 묘사나 실시간 API 데이터 통신 규격 정의는 정보 객체의 설계 목적이 아니므로 오답입니다.\n\n**[Exam Keywords]**\nRole, CMDB, CSDM, Compliance, Business Application, Information Object, GDPR, HIPAA"
  },
  {
    "id": 138,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB 건강도(Health) 아키텍처 표준 가이드라인에 따라, 클래스 단위가 아닌 비즈니스/지리적 컨텍스트(지역, 부서 등)로 건강도 점수를 쪼개어 연산하려면 반드시 'CMDB Group' 프레임워크를 활용해야 합니다. 그룹 생성 시 유형(Type)을 'health'로 지정하면, 대시보드 스캔 엔진이 해당 리전 그룹용 스코어카드를 독립 연산하여 표출해 줍니다. 스케줄러 잡 코드 개조는 업그레이드를 파괴하는 오답이며, 단일 토글 스위치는 플랫폼 UI에 실존하지 않는 허구의 옵션이므로 오답입니다.\n\n**[Exam Keywords]**\nGroup, Groups, CMDB, CMDB Health Dashboard, APAC, CI, UI"
  },
  {
    "id": 139,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow IRE(식별 및 조정 엔진) 아키텍처 가이드라인에 따라, 다중 수집 출처 간의 속성 필드 업데이트 권한 및 우선순위 통제는 'CI Class Manager' 내부의 'Reconciliation Rules(조정 규칙)' 탭에서 전담 설계합니다. 자산 레코드 간의 동일성 매칭 기준을 수립하는 곳은 Identification Rule 탭이며, 클래스의 레이블이나 테이블 이름을 정의하는 방은 Basic Info 탭이므로 속성 업데이트 권한 맥락에서 모두 오답입니다.\n\n**[Exam Keywords]**\nReconciliation Rules tab"
  },
  {
    "id": 140,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow 데이터 무결성 표준 명세에 따라, 특정 인입 출처가 CMDB 마스터 레코드의 클래스 정보나 세부 속성 필드 세트를 업데이트할 수 있도록 권한을 통제하는 전담 컴포넌트는 'Reconciliation Rule(조정 규칙)'입니다. 인입 자산의 중복 여부 및 신원 확인만 수행하는 것은 Identification Rules이며, 중복 발생 시 발급되는 티켓은 De-duplication Task입니다. 은 시스템에 존재하지 않는 허구의 용어이므로 오답입니다.\n\n**[Exam Keywords]**\nReconciliation Rule"
  },
  {
    "id": 141,
  ...
  },
  {
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB 360(Multisource) 거버넌스 가이드라인에 따라, 소스별 원본 수집 페이로드를 영구 적재하는 멀티소스 데이터 모델은 대량의 스토리지 공간을 요구하므로 순정 상태(Base System)에서 기본 활성화되어 있지 않습니다. 반드시 어드민이 관련 핵심 시스템 속성인 'glide.identification_engine.multisource_enabled' 값을 true로 명시적 전환해야 가동됩니다. Class Models 플러그인은 스키마 확장 도구일 뿐이므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB, CMDB 360, Multisource CMDB, Discovery"
  },
  {
    "id": 143,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow 데이터 모델 규격에 따라, 사용자(Users), 그룹(Groups), 위치(Locations) 등은 CMDB에 종속된 자산 객체가 아니며 플랫폼 전역 프로세스가 공유하는 'Foundational Data(기반 데이터)'에 해당합니다. CMDB의 구성 항목(CI)들은 이 기반 데이터들을 참조 필드를 통해 유기적으로 연결하여 컨텍스트를 완성합니다. 자산 저장소 자체, 필드 스키마 정의, 확장 테이블 분기 구조는 기반 데이터의 정의를 충족하지 못하므로 오답입니다.\n\n**[Exam Keywords]**\nFoundational Data"
  },
  {
    "id": 144,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow 플랫폼 표준 명세에 따라, 이름(Name), 시리얼 번호(Serial Number), 운영체제(Operating System)처럼 개별 구성 항목(CI)의 내부 스펙을 구체적으로 묘사하는 데이터 필드 조항들을 'Attributes(속성)'라고 부릅니다. 데이터베이스 전체를 아우르는 용어는 CMDB이며, 외부의 공동 마스터 레코드는 Foundational Data, 테이블 단위의 뼈대는 Class에 해당하므로 모두 오답입니다.\n\n**[Exam Keywords]**\nAttributes"
  },
  {
    "id": 145,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB 데이터 모델 표준 명세에 따라, 공통된 속성(Attributes) 스펙을 공유하는 특정 자산 유형의 집합을 물리적으로 관리하는 데이터베이스 테이블 단위를 'Class(클래스)'라고 부릅니다. 예를 들어 Windows Server나 IP Switch 등은 독립된 하나의 클래스 테이블로 취급됩니다. 데이터베이스 전체 에코시스템을 뜻하는 CMDB, 개별 필드 명세인 Attributes, 참조용 조직 데이터인 Foundational Data는 모두 클래스의 정의 영역을 벗어나므로 오답입니다.\n\n**[Exam Keywords]**\nClass"
  },
  {
    "id": 146,
    ...
    "explanation": "**[핵심 근거]**\n> ServiceNow 데이터 거버넌스 표준 아키텍처에 따라, 스토어 전용 앱인 CMDB Data Foundations Dashboard는 '데이터 관리 프랙티스' 탭을 통해 최근 90일 이내 데이터 최신성과 수집 데이터의 IRE 엔진 통과율 등을 핵심 메트릭 지표로 추적합니다. 반면, 필수 관계가 끊어진 고아 자산 식별 전용 보고서와 핵심 감사 규칙 실패에 따른 개별 수정 작업(Task) 시트 생성은 모두 전통적인 플랫폼 순정 기능인 **'CMDB Health Dashboard'의 고유 영역(Correctness 지표 등)**이므로 오답입니다\n\n**[Exam Keywords]**\nData Foundations Dashboard, CMDB Health Dashboard, Correctness, Active CIs, IRE Metrics"
  },
  {
    "id": 147,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB 데이터 정제 거버넌스 표준 명세에 따라, 자산을 시스템 상에서 완전히 삭제(Delete)하거나 아카이브(Archive) 테이블로 격리하는 정책은 데이터 무결성을 보호하기 위해 반드시 해당 자산이 공식 퇴역 상태인 'End of Life - Retired' 상태에 안착해 있을 때만 가동됩니다. 감사 추적이 필요한 도난 자산이나 즉시 서비스 배포가 가능한 가용 재고 자산을 무단 정리할 경우 데이터 오염 및 서비스 장애를 유발하므로 오답이며, 은 플랫폼 아키텍처 상 허용되지 않는 위험한 접근이므로 오답입니다.\n\n**[Exam Keywords]**\nEnd of Life - Retired"
  },
  {
    "id": 148,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CSDM 거버넌스 데이터 모델 표준 규격에 따라, 개별 인프라 자산(CI)들의 실실적인 운영 담당 그룹(Support Group, Change Group 등)을 중앙 집중식으로 동기화하고 자동 관리하는 본산은 'Technology Management Offering (구 Technical Service Offering)'입니다. 오퍼링 레벨에 지정된 지원 그룹 정보는 하부 관계형 자산들로 자동 전파되도록 플랫폼 아키텍처가 설계되었습니다. 상위 개념 분류단인 Technology Management Service (구 Technology Management Service (구Technical Service))나 동적 컨테이너인 Dynamic CI Group은 속성 자동 전파 및 유지 기능을 전담하지 않으므로 오답입니다.\n\n**[Exam Keywords]**\nTechnology Management Service Offering (Technology Management Offering (구 Technical Service Offering))"
  },
  {
    "id": 149,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow 라이프사이클 마이그레이션 아키텍처 표준 명세에 따라, Enable Life Cycle Sync를 수행한 직후 시스템은 데이터 검증을 통해 'Discrepancy Report(불일치 리포트)'를 발행합니다. 어드민은 데이터 오염을 예방하기 위해 미완성되거나 모순된 매핑 내역을 먼저 해결해야 합니다. 정합성을 확보하기 전에 성급하게 매핑을 전면 가동하면 심각한 데이터 충돌이 발생하며, 표준 구조를 레거시 수준으로 역변환하는 행위 또한 아키텍처 가이드라인에 위배되므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB, CSDM"
  },
  {
    "id": 150,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow 데이터 통합 거버넌스 명세에 따라, IntegrationHub ETL은 시각적 매핑 UI를 제공하며 CMDB뿐만 아니라 사용자, 위치 등 일반 Non-CMDB 테이블로의 데이터 적재도 폭넓게 지원합니다. 또한 적재 과정에서 반드시 백엔드 식별 및 조정 엔진(IRE)을 경유하도록 강제하여 중복 자산 방지 및 데이터 무결성을 보장합니다. 스스로 네트워크를 스캔하는 Discovery 엔진이 아니며, 특정 MS 제품 전용 한정 수입 도구도 아니므로 오답입니다.\n\n**[Exam Keywords]**\nCMDB, IRE, IntegrationHub ETL, Discovery, ETL, UI, MS"
  },
  {
    "id": 151,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CSDM 거버넌스 및 ITSM 연동 표준 명세에 따라, 비즈니스 중단 및 매출 리스크에 맞춰 장애 우선순위를 동적으로 차등화하는 마스터 속성은 서비스 오퍼링 레벨에 존재하는 'Business Criticality' 필드입니다. 플랫폼은 이 값을 참조하여 인시던트의 Impact/Urgency 및 SLA를 상향 조정합니다. 단순 엔지니어 배정 부서, 장애 영향 자산 목록, 서비스 테이블의 종류를 식별하는 스키마 구분 필드는 비즈니스 가치 기반 우선순위 제어권을 가지지 못하므로 오답입니다.\n\n**[Exam Keywords]**\nCSDM, Application Service, ITSM, SLA"
  },
  {
    "id": 152,
  ...
  },
  {
    ...
    "explanation": "**[핵심 근거]**\nServiceNow 데이터 모델 규격에 따라 개별 인프라 자산의 관리 주체(Managed by group) 속성을 대량 동기화하는 경로는 이원화되어 있습니다. 첫째, 테이블 스키마 차원에서 고정 규칙을 부여하는 'CI Class Manager'가 있으며, 둘째, CSDM 관계선을 통해 하부 종속 자산들로 그룹 컨텍스트를 동적 상속 전파하는 'Technology Management Offerings'가 정답입니다. 구체적 속성이 존재하지 않는 추상적인 Service 단이나, 인프라 엔지니어링 도메인이 아닌 대고객 비즈니스 계층은 동기화 원천이 될 수 없으므로 오답입니다.\n\n**[Exam Keywords]**\nCI Class Manager, Technology Management Offerings (Technology Management Offering (구 Technical Service Offering)s)"
  },
  {
    "id": 154,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CSDM 5.0 거버넌스 표준에 따라, 엔터프라이즈 아키텍트(EA)는 기업의 비즈니스 역량(Business Capabilities)과 비즈니스 애플리케이션(Business Applications) 청사진을 기획하는 'Design & Planning' 도메인의 핵심 오너입니다. Ideation은 전략 기획, Build/Integration 및 Service Delivery는 실무 구현과 운영, Consumption은 서비스 소비를 담당하므로 EA의 핵심 설계 통제 도메인인 Design & Planning이 정답입니다.\n\n**[Exam Keywords]**\nDesign & Planning (Design Domain)"
  },
  {
    "id": 155,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow 플랫폼 거버넌스 표준 명세에 따라 CSDM 청사진을 무시하고 임의 커스텀을 남발할 경우, 유지보수 비용을 폭증시키는 'Technical Debt(기술 부채)'가 기하급수적으로 증가합니다. 또한 차세대 릴리스나 신규 고도화 제품군들은 CSDM 스키마의 완벽한 정합성을 전제로 구동되므로 미래 제품과의 'Incompatibility(호환성 상실)' 리스크를 유발합니다. 설치 프로세스 자체를 차단하거나 데이터 물리적 증발을 일으키는 것은 아니므로 오답입니다.\n\n**[Exam Keywords]**\nCSDM, IRE, Spoke"
  },
  {
    "id": 156,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CSDM 데이터 거버넌스 표준 명세에 따라, 실제 프로덕션 환경에 배포된 서비스 인스턴스(Service Instance (구Application Service)) 및 이와 연동되어 자동 탐지 엔진(Discovery)에 의해 수집·관리되는 하부 인프라 기술 자산(CIs) 간의 유기적 지형도를 관제하는 본산은 'Service Delivery (구 Manage Technology Management Services (구Technical Services))' 도메인입니다. 추상적인 앱 청사진만 기획하는 Design이나, 카탈로그 소비를 관장하는 Consumption, 그리고 CSDM 도메인이 아닌 프로세스성 명사들은 실시간 배포 자산 탐지 도메인을 충족하지 못하므로 오답입니다.\n\n**[Exam Keywords]**\nService Delivery (구 Manage Technology Management Services (구Technical Services)) 도메인"
  },
  {
    "id": 157,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CSDM 거버넌스 표준 명세에 따라, 외부 고객(Customers) 및 비즈니스 사용자와 소통하며 최종 비즈니스 가치와 서비스 계약을 인도·소비하는 영역은 'Sell / Consume (Service Consumption)' 도메인입니다. 따라서 외부 고객 포탈 및 케이스 관리를 전담하는 Customer Service Management(CSM) 솔루션은 이 도메인에 완벽히 매핑됩니다. 엔지니어 중심의 운영 레이어인 Service Delivery나 아키텍처 기획단인 Design은 고객 서비스 소비를 직접 관장하지 않으므로 오답입니다.\n\n**[Exam Keywords]**\nService Consumption (Sell / Consume Domain)"
  },
  {
    "id": 158,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CSDM 운영 거버넌스 표준 명세에 따라, 실제 런타임 환경에 배포된 소프트웨어 스택과 그 하부 인프라 의존성 관계선들을 논리적인 서비스 단위로 묶어 모니터링 및 ITSM 영향도 분석의 교두보 역할을 하는 컴포넌트는 'Service Instance (구Application Service) [cmdb_ci_service_auto]'입니다. 아키텍처 기획 문서 역할을 하는 Business Application이나 단일 소프트웨어 인스턴스 조각인 Application, 그리고 운영 부서의 묶음 분류인 Technology Management Service (구 Technology Management Service (구Technical Service))는 배포된 스택의 전체 의존성을 논리적으로 대변하는 인스턴스가 아니므로 오답입니다.\n\n**[Exam Keywords]**\nService Instance (구Application Service) [cmdb_ci_service_auto]"
  },
  {
    "id": 159,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CSDM 및 제품 포트폴리오 거버넌스 표준 명세에 따라 기획(Plan), 개발(Build), 운영(Run) 단계를 넘나들며 포트폴리오, 서비스, 오퍼링, 제품의 전 수명주기 건강도를 단일 창으로 통합 모니터링 및 제어하는 독점 워크스페이스는 'Digital Portfolio Management (DPM)'입니다. 실시간 장애 관제 및 헬프데스크 운영에 특화된 Service Operations Workspace나 자산 탐지·검색을 위한 CMDB Workspace, 그리고 단순 생성기인 Service Builder는 전사 포트폴리오 라이프사이클 통합 분석을 제공하지 않으므로 오답입니다.\n\n**[Exam Keywords]**\nDigital Portfolio Management (DPM)"
  },
  {
    "id": 160,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow 서비스 카탈로그 및 CSDM 거버넌스 표준 명세에 따라, 비즈니스/기술 서비스 및 오퍼링 개체들을 복잡한 테이블 폼 수동 조작 없이 마법사 기반 UI를 통해 패키지 형태로 신속하게 빌드·연결하는 통합 유틸리티는 'Service Builder'입니다. 자산 탐색 중심의 CMDB Workspace, 기술적 자산 토폴로지를 스캔하는 Service Mapping, 오직 배포판 인스턴스 전용 생성 툴인 Service Instance (구Application Service) Wizard는 이 포괄적인 서비스 포트폴리오 상품 디자인 능력을 제공하지 않으므로 오답입니다.\n\n**[Exam Keywords]**\nService Builder"
  },
  {
    "id": 161,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CSDM 관계성 거버넌스 표준 명세에 따라, 실시간 가동 중인 개별 Service Instance (구Application Service) 레코드를 허브 삼아 상위 비즈니스 애플리케이션(기획), 기술 서비스 오퍼링(운영), 비즈니스 서비스 오퍼링(소비) 간의 다차원 관계선을 일괄 매핑하고 조립하는 전담 구성 워크플로우 헬퍼는 'Service Instance (구Application Service) Wizard'입니다. 서비스 포트폴리오 카탈로그 구조 자체를 빌드하는 Service Builder나 스키마를 설계하는 CI Class Manager는 개별 배포 인스턴스 중심의 CSDM 다차원 관계 결합 기능을 제공하지 않으므로 오답입니다.\n\n**[Exam Keywords]**\nService Instance (구Application Service) Wizard"
  },
  {
    "id": 162,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow 표준 ITSM 및 CSDM 거버넌스 연동 명세에 따라, 실시간 장애를 복구하는 Incident 티켓의 Assignment group은 자산의 수리를 담당하는 'Support group'에서 자동으로 값을 상속받습니다. 반면, 인프라 변경 및 패치 작업을 관리하는 Change Request 티켓의 Assignment group은 자산의 작업 그룹을 대변하는 'Change group' 속성에서 값을 자동으로 위임받아 채워집니다. 자산 데이터 장부 관리자인 Managed by group이나 결재 승인 부서인 Approval group은 티켓의 실무 처리 부서로 자동 배정되지 않으므로 오답입니다.\n\n**[Exam Keywords]**\nGroup, CSDM, ITSM, CI"
  },
  {
    "id": 163,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CSDM 및 ITSM 거버넌스 표준 명세에 따라, 실시간 장애 및 변경 작업의 표적이 되는 메인 Configuration item 필드에는 운영 상태 추적 및 지원 그룹 매핑이 가능한 'Operational CIs'가 상주해야 합니다. 따라서 라이브 배포판 인스턴스인 Service Instance (구Application Service)와 대량 인프라 제어 통로인 Dynamic CI Group이 완벽한 정답입니다. 개념적 기획서에 불과한 Business Application이나 서버 내부의 단순 참조 스캔 데이터인 Installed Software는 독립적인 운영 타깃 CI로 직접 유입될 수 없으므로 오답입니다.\n\n**[Exam Keywords]**\nDynamic CI Group [cmdb_ci_query_based_service], Service Instance (구Application Service) [cmdb_ci_service_auto]"
  },
  {
    "id": 164,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow 데이터 거버넌스 라이프사이클 표준 명세에 따라, 데이터 파운데이션을 성취하기 위한 3대 기둥(3 Pillars)은 데이터를 표준 인입하는 'Ingest', 무결성 가드레일 규칙을 강제하는 'Govern', 그리고 파운데이션 대시보드와 플레이북으로 처방을 내리는 'Insight'입니다. 선지에 배치된 Completeness, Correctness, Compliance는 CMDB Health Dashboard의 품질 측정용 3대 스코어카드(3 Cs) 분류 명칭일 뿐, 파운데이션 수명주기를 관장하는 3대 기둥 프레임워크가 아니므로 오답입니다.\n\n**[Exam Keywords]**\nInsight, Ingest, Govern"
  },
  {
    "id": 165,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow 데이터 컴플라이언스 및 거버넌스 표준 명세에 따라, 인스턴스 전역의 자산 관계 모델이 CSDM 모범 사례(Good Practices) 규격에 부합하는지 정량 진단하고, Crawl-Walk-Run-Fly 성숙도 탭을 통해 가시성과 개선 플레이북을 독점 제공하는 내장 분석 도구는 'CSDM Data Foundations Dashboard'입니다. 자산 자체의 데이터 품질 무결성(3Cs)만 측정하는 CMDB Health Dashboard나, 개별 관계선을 수동 조립하는 Wizard, 상품을 생성하는 Builder는 CSDM 포트폴리오 정렬도 진단이라는 고유 목적을 충족하지 못하므로 오답입니다.\n\n**[Exam Keywords]**\nCSDM Data Foundations Dashboard"
  },
  {
    "id": 166,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CSDM 라이프사이클 거버넌스 표준 명세에 따라, 플랫폼의 모든 구성 항목(CI) 자산의 탄생부터 퇴역까지 일관된 방식으로 이 단계(Dual-tier) 매트릭스로 관리하도록 권장하는 공식 필드 쌍은 'Life cycle stage'와 'Life cycle stage status'입니다. 선지에 제시된 Hardware status, Install status, Operational status는 과거에 쓰이던 파편화된 레거시 필드들이며, 차세대 마이그레이션 및 동기화 엔진을 통해 표준 필드로 통합 변환되어야 하는 대상이므로 오답입니다.\n\n**[Exam Keywords]**\nLife cycle stage and stage status"
  },
  {
    "id": 167,
  ...
  },
  {
    ...
    "explanation": "**[핵심 근거]**\nCSDM 5.0에서 새롭게 추가된 'Ideation & Strategy' 도메인은 제품의 아이디어 제안, 전략적 우선순위 설정, 목표 및 타겟 정의 등 수명 주기의 가장 초기 단계를 담당합니다. 이 도메인은 Strategic Portfolio Management(SPM) 제품군과 밀접하게 연동되어 비즈니스 전략과 IT 실행을 연결하는 역할을 합니다. Design & Planning은 논리적 설계, Manage Portfolios는 전체 포트폴리오 가치 관리, Service Delivery는 운영 인프라를 담당하므로 오답입니다.\n\n**[Exam Keywords]**\nIdeation & Strategy"
  },
  {
    "id": 169,
    ...
    "explanation": "**[핵심 근거]**\nCSDM 5.0은 보다 직관적인 명칭 사용을 위해 도메인 이름을 변경했습니다. 과거 4.0에서 IT 내부 인프라와 기술적 의존성을 관리하던 'Manage Technical Services' 도메인은 이제 'Service Delivery'라는 명칭으로 통합되었습니다. 마찬가지로 Service Consumption (구 Sell/Consume)은 Service Consumption으로, Design은 Design & Planning으로, Build는 Build & Integration으로 변경되었습니다.\n\n**[Exam Keywords]**\nService Delivery"
  },
  {
    "id": 170,
    ...
    "explanation": "**[핵심 근거]**\nCSDM 5.0에서 cmdb_ci_service_auto 테이블의 새 명칭은 **Service Instance**입니다. **Application Service**는 기존 이름이고, **Service Instance**가 새 레이블을 정확히 가리킵니다. 이 변경은 AI, 네트워크, 시설(Facility) 같은 비-IT 서비스 인스턴스까지 함께 다루기 위한 확장입니다.\n\n**[Exam Keywords]**\nService Instance"
  },
  {
    "id": 171,
    ...
    "explanation": "**[핵심 근거]**\nCSDM 5.0은 AI 거버넌스를 위해 AI Function(외부 서비스), AI Application(내부 구축 앱), AI Model 클래스를 도입했습니다. 이를 통해 Now Assist와 같은 AI 에이전트의 기술적 의존성을 CMDB에서 추적할 수 있습니다.\n\n**[Exam Keywords]**\nAI Function, AI Application, AI Model"
  },
  {
    "id": 172,
    ...
    "explanation": "**[핵심 근거]**\nSBOM은 디지털 제품을 구성하는 모든 소프트웨어 컴포넌트의 명세서 역할을 합니다. 이를 통해 보안 취약점 발생 시 즉각적인 영향도 분석이 가능하며, 소프트웨어 공급망의 보안성을 강화하는 것이 핵심 목적입니다.\n\n**[Exam Keywords]**\nTo enhance vulnerability management and software supply chain security"
  },
  {
    "id": 173,
    ...
    "explanation": "**[핵심 근거]**\nCSDM 5.0의 Digital System Model은 제품을 비즈니스 프로세스, 서비스(논리), 기능, 인프라의 4개 계층으로 나누어 수직/수평적 종속성을 가시화합니다.\n\n**[Exam Keywords]**\nBusiness Process, Service (Logical), Functional, Infrastructure"
  },
  {
    "id": 174,
    ...
    "explanation": "**[핵심 근거]**\nCSDM 5.0에서 Manage Portfolios 도메인은 기존 Design 도메인에서 분리되었습니다. 이 도메인은 DPM(Digital Portfolio Management)과 연동하여 서비스와 제품의 비즈니스 가치 및 성과를 전문적으로 관리합니다.\n\n**[Exam Keywords]**\nManage Portfolios"
  },
  {
    "id": 175,
    ...
    "options": [
      {
        "id": "A",
        "text": "A Windows Server IS allowed to be downgraded"
      },
      {
        "id": "B",
        "text": "A Windows Server IS NOT allowed to be downgraded"
      },
      {
        "id": "C",
        "text": "The Windows Server CI attributes CAN be updated"
      },
      {
        "id": "D",
        "text": "The Windows Server CI attributes CANNOT be updated"
      }
    ],
    "answer": [
      "B",
    ...
    "explanation": "**[핵심 근거]**\n글로벌 속성인 'glide.identification_engine.update_without_downgrade_enabled'는 'glide.class.downgrade.enabled'보다 우선순위가 높습니다. 따라서 클래스 재분류(다운그레이드)는 허용되지 않지만, CI 속성에 대한 업데이트는 허용됩니다.\n\n**[Exam Keywords]**\nA Windows Server IS NOT allowed to be downgraded, CI, IS, NOT, CAN"
  },
  {
    "id": 176,
    ...
    "options": [
      {
        "id": "A",
        "text": "A Windows Server IS allowed to be downgraded to a Computer record"
      },
      {
        "id": "B",
        "text": "A Windows Server IS NOT allowed to be downgraded to a Computer record"
      },
      {
        "id": "C",
        "text": "The Windows Server CI attributes CAN be updated"
    ...
    "explanation": "**[핵심 근거]**\n재분류 제한 규칙(Reclassification restriction rules)은 글로벌 속성인 'glide.class.downgrade.enabled'보다 우선하여 적용됩니다. 규칙에 의해 다운그레이드 재분류는 차단되지만, CI 속성 업데이트는 여전히 가능합니다.\n\n**[Exam Keywords]**\nA Windows Server IS NOT allowed to be downgraded to a Computer record, CI, IS, NOT, CAN"
  },
  {
    "id": 177,
    ...
    "options": [
      {
        "id": "A",
        "text": "An IP Router IS allowed to be downgraded to a Network Gear record"
      },
      {
        "id": "B",
        "text": "An IP Router IS NOT allowed to be downgraded to a Network Gear record"
      },
      {
        "id": "C",
        "text": "The IP Router CI attributes CAN be updated"
      },
      {
        "id": "D",
        "text": "The IP Router CI attributes CANNOT be updated"
      }
    ],
    "answer": [
      "A",
    ...
    "explanation": "**[핵심 근거]**\nIP Router와 Network Gear 클래스 쌍에 대해 정의된 구체적인 재분류 제한 규칙이 없으므로, 글로벌 속성인 'glide.class.downgrade.enabled'가 적용됩니다. 이에 따라 다운그레이드 재분류와 CI 속성 업데이트가 모두 허용됩니다.\n\n**[Exam Keywords]**\nAn IP Router IS allowed to be downgraded to a Network Gear record, IP, CI, IS, CAN"
  },
  {
    "id": 178,
    ...
    "explanation": "**[핵심 근거]**\n운영 체제(Operating System), RAM, MAC 주소는 Discovery를 통해 수집되는 기술 데이터 포인트입니다. 반면 Support Group과 Managed by Group은 Discovery로 탐지할 수 없는 속성이며, 다른 방법을 통해 입력되어야 합니다.\n\n**[Exam Keywords]**\nOperating System, RAM, MAC Address"
  },
  {
    "id": 179,
    ...
    "options": [
      {
        "id": "A",
        "text": "Provides insights into the potential impact of changes"
      },
      {
        "id": "B",
        "text": "Helps identify the root cause of incidents faster, reducing service downtime"
      },
      {
        "id": "C",
        "text": "Provides an up to date inventory of all infrastructure in an enterprise environment reducing compliance risk"
      },
      {
        "id": "D",
        "text": "Gives visibility to all installed software in an enterprise supporting corporate software asset management initiatives"
      }
    ],
    "answer": [
      "A",
    ...
    "explanation": "**[핵심 근거]**\nService Mapping은 인프라 구성 요소를 특정 비즈니스 서비스에 매핑하는 데 중점을 두며, 이를 통해 변경 사항의 잠재적 영향 분석과 인시던트 근본 원인 파악을 가속화하여 다운타임을 줄여줍니다.\n\n**[Exam Keywords]**\nService Mapping, reducing service downtime"
  },
  {
    "id": 180,
    ...
    "explanation": "**[핵심 근거]**\nAgent Client Collector(ACC)는 ServiceNow에서 제공하는 경량 에이전트 기반 솔루션으로, 조직 전반의 서버, 클라우드 리소스 및 애플리케이션에 대한 실시간 모니터링과 디스커버리를 수행합니다.\n\n**[Exam Keywords]**\nAgent Client Collector"
  },
  {
    "id": 181,
    ...
    "explanation": "**[핵심 근거]**\nService Graph Connectors와 IntegrationHub ETL은 데이터 무결성을 보장하면서 서드파티 데이터를 ServiceNow CMDB 또는 일반 테이블에 통합하는 데 사용되는 핵심 도구입니다. 두 도구 모두 식별 및 조정 엔진(IRE)을 활용합니다.\n\n**[Exam Keywords]**\nService Graph Connectors, IntegrationHub ETL"
  },
  {
    "id": 182,
    ...
    "explanation": "**[핵심 근거]**\n데이터가 IRE를 거치도록 하려면 변환 맵(Transform Map) 내에 'onBefore' 스크립트를 작성하여 CMDBTransformUtil API를 호출해야 합니다.\n\n**[Exam Keywords]**\nonBefore"
  },
  {
    "id": 183,
    ...
    "options": [
      {
        "id": "A",
        "text": "Value from CI Class Manager"
      },
      {
        "id": "B",
        "text": "Value from Technology Management Offering"
      }
    ],
    "answer": [
      "B"
    ],
    "explanation": "**[핵심 근거]**\nCSDM 가이드라인에 따라, 기술 서비스 오퍼링(Technology Management Offering)에 구성된 값이 CI Class Manager에 설정된 기본값보다 우선하여 적용됩니다.\n\n**[Exam Keywords]**\nValue from Technology Management Offering"
  },
  {
    "id": 184,
    ...
    "options": [
      {
        "id": "A",
        "text": "Change group, Managed by group, and Support group"
      },
      {
        "id": "B",
        "text": "Owned by, Managed by, Assigned to"
      },
      {
        "id": "C",
        "text": "Change group, Managed by group, and Approval group"
      },
      {
        "id": "D",
        "text": "Owned by, Managed by, Supported by"
      }
    ],
    "answer": [
      "A"
    ],
    "explanation": "**[핵심 근거]**\nChange group, Managed by group, Support group은 기술 서비스 오퍼링을 통해 관리될 때 하위 CI들과 자동으로 동기화되는 핵심 기반 데이터입니다.\n\n**[Exam Keywords]**\nChange group, Managed by group, and Support group"
  },
  {
    "id": 185,
    ...
    "options": [
      {
        "id": "A",
        "text": "CIs discovered by ServiceNow Discovery have a discovery source field of ServiceNow."
      },
      {
        "id": "B",
        "text": "CIs discovered by ServiceNow Discovery have a discovery source field of Service Mapping."
      },
      {
        "id": "C",
        "text": "CIs added to the CMDB through the Asset Management process have a discovery source field of SNAssetManagement."
      },
      {
        "id": "D",
        "text": "CIs added to the CMDB through the Asset Management process have a discovery source field of AssetManagement"
      }
    ],
    "answer": [
      "A",
    ...
    "explanation": "**[핵심 근거]**\nServiceNow Discovery는 Discovery Source 필드를 'ServiceNow'로 설정하고, 자산 관리 프로세스를 통해 추가된 자산은 'SNAssetManagement'로 설정됩니다.\n\n**[Exam Keywords]**\nCMDB, Discovery, CI"
  },
  {
    "id": 186,
    ...
    "options": [
      {
        "id": "A",
        "text": "Reconciliation Rules - Where you configure data sources authorized to update CI Attributes"
      },
      {
        "id": "B",
        "text": "Basic Info - Where you configure an icon for a specific class"
      },
      {
        "id": "C",
        "text": "Pinned Classes - What feature allows for easy navigation back to a frequently accessed class"
      },
      {
        "id": "D",
        "text": "Identification Rules - Where you go to configure a rule to uniquely identify CIs"
      }
    ],
    "answer": [
      "A",
    ...
    "explanation": "**[핵심 근거]**\nCI Class Manager에서: Reconciliation Rules 탭은 승인된 데이터 소스를 구성하고, Basic Info 탭은 클래스 아이콘/설정을 담당하며, Pinned Classes는 빠른 탐색을 지원하고, Identification Rule 탭은 CI를 고유하게 식별하는 규칙을 정의합니다.\n\n**[Exam Keywords]**\nCI Class Manager, Identification Rule, Identification Rules, Reconciliation Rule, Reconciliation Rules, CI"
  },
  {
    "id": 187,
    ...
    "options": [
      {
        "id": "A",
        "text": "Updates to a CI"
      },
      {
        "id": "B",
        "text": "Inserts of a CI"
      },
      {
        "id": "C",
        "text": "Deletion of a CI"
      },
      {
        "id": "D",
        "text": "Reading of a CI"
      }
    ],
    "answer": [
      "A"
    ],
    "explanation": "**[핵심 근거]**\n조정 규칙(Reconciliation rules)은 서로 다른 데이터 소스들로부터 유입되는 CI 속성에 대한 업데이트(수정) 권한을 제어하도록 설계되었습니다.\n\n**[Exam Keywords]**\nReconciliation Rule, Reconciliation Rules, CI"
  },
  {
    "id": 188,
    ...
    "options": [
      {
        "id": "A",
        "text": "Reconciliation Rules - Manage attribute updates (속성 업데이트 관리)"
      },
      {
        "id": "B",
        "text": "IRE Data Source Rules - Control CI insert (CI 삽입 제어)"
      },
      {
        "id": "C",
        "text": "De-duplication Tasks - Process duplicate records (중복 레코드 처리)"
      },
      {
        "id": "D",
        "text": "Identification Rules - Define CI uniqueness (CI의 고유성 정의)"
      }
    ],
    "answer": [
      "A",
    ...
    "explanation": "**[핵심 근거]**\n조정 규칙(Reconciliation Rules)은 속성 업데이트를 관리하고, IRE 데이터 소스 규칙(Data Source Rules)은 CI 삽입을 제어하며, 중복 제거 작업(De-duplication Tasks)은 중복 레코드를 처리하고, 식별 규칙(Identification Rules)은 CI의 고유성을 정의합니다.\n\n**[Exam Keywords]**\nReconciliation Rules, IRE Data Source Rules, De-duplication Tasks, Identification Rules, CI Insert, CI Uniqueness"
  },
  {
    "id": 189,
    ...
    "explanation": "**[핵심 근거]**\n동적 조정 규칙이 'Largest Value(가장 큰 값)'로 설정되어 있으므로, 사용 가능한 소스 중 가장 높은 RAM 값인 6020이 선택되어 기록됩니다.\n\n**[Exam Keywords]**\n6020"
  },
  {
    "id": 190,
    ...
    "options": [
      ...
      {
        "id": "B",
        "text": "Activate the Multisource CMDB by setting the property, glide.identification_engine.multisource_enabled, to true and installing the ITOM Discovery License plugin"
      },
      {
        "id": "C",
      ...
      {
        "id": "D",
        "text": "Must be activated on base systems"
      }
    ],
    "answer": [
      "B",
    ...
    "explanation": "**[핵심 근거]**\nMultisource CMDB는 기본적으로 활성화되어 있지 않으며, 'glide.identification_engine.multisource_enabled' 속성을 true로 설정하고 적절한 ITOM Discovery 라이선스 플러그인을 설치하여 명시적으로 활성화해야 합니다.\n\n**[Exam Keywords]**\nCMDB, Multisource CMDB, Discovery, ITOM"
  },
  {
    "id": 191,
    ...
    "explanation": "**[핵심 근거]**\n'Most reported(최빈값)' 로직은 가장 자주 나타나는 값을 선택합니다. 이 시나리오에서는 4096이 두 번(Tivoli, ServiceNow) 나타나므로 최빈값인 4096이 선택됩니다.\n\n**[Exam Keywords]**\n4096"
  },
  {
    "id": 192,
    ...
    "options": [
      ...
      {
        "id": "C",
        "text": "Dynamic Reconciliation Rule"
      },
      {
        "id": "D",
        "text": "Data Refresh Rule"
    ...
    "explanation": "**[핵심 근거]**\nIRE 데이터 소스 규칙(IRE Data Source Rules)은 특정 데이터 소스가 특정 CMDB 클래스에 신규 레코드를 삽입(Insert)하는 것을 제어하거나 차단하기 위해 사용되는 전용 규칙입니다.\n\n**[Exam Keywords]**\nIRE Data Source Rule"
  },
  {
    "id": 193,
    ...
    "explanation": "**[핵심 근거]**\n통합 맵(Unified Map)은 애플리케이션 서비스 맵과 의존성 뷰(Dependency View)의 기능을 통합하여 CI 간의 관계를 종합적으로 시각화해 줍니다.\n\n**[Exam Keywords]**\nApplication Service, Dependency View"
  },
  {
    "id": 194,
    ...
    "explanation": "**[핵심 근거]**\n통합 맵(Unified Map)은 CMDB Workspace 내부에 통합되어 있어 해당 워크스페이스에서 직접 접근하고 사용할 수 있습니다.\n\n**[Exam Keywords]**\nCMDB Workspace"
  },
  {
    "id": 195,
    ...
    "explanation": "**[핵심 근거]**\nCMDB Query Builder를 사용하면 복잡한 데이터베이스 쿼리를 시각적으로 설계할 수 있으며, 특정 CI(예: 데이터 센터)와 연결 관계가 없는 항목들을 식별하는 데 매우 유용합니다.\n\n**[Exam Keywords]**\nCMDB Query Builder"
  },
  {
    "id": 196,
    ...
    "explanation": "**[핵심 근거]**\n지능형 검색(Intelligent Search)은 자연어 쿼리(NLQ, Natural Language Query)를 활용하여 사용자가 일상적인 용어로 검색할 수 있도록 지원합니다.\n\n**[Exam Keywords]**\nNLQ"
  },
  {
    "id": 197,
    ...
    "options": [
      {
        "id": "A",
        "text": "CMDB and CSDM Data Foundations Dashboards"
      },
      {
        "id": "B",
        "text": "Data Manager"
    ...
    "explanation": "**[핵심 근거]**\n공통적인 CMDB 및 CSDM 이슈를 해결하는 데 도움을 주는 치유 플레이북(Remediation Playbooks)은 CMDB/CSDM Data Foundations Dashboard 스토어 앱을 통해 제공됩니다.\n\n**[Exam Keywords]**\nCMDB and CSDM Data Foundations Dashboards"
  },
  {
    "id": 198,
    ...
    "explanation": "**[핵심 근거]**\n'Support Group' 필드는 인시던트(Incident)의 담당 그룹으로, 'Change Group' 필드는 변경(Change) 요청의 담당 그룹으로 자동 매핑되어 효율적인 티켓 라우팅을 돕습니다.\n\n**[Exam Keywords]**\nSupport Group, Change Group"
  },
  {
    "id": 199,
    ...
    "options": [
      {
        "id": "A",
        "text": "Allows them to test for required fields that are missing data"
      },
      {
        "id": "B",
        "text": "Allows them to filter which CI classes are calculated as part of the CMDB Health Dashboard"
      },
      {
        "id": "C",
        "text": "Allows them to identify and filter duplicate CIs"
      },
      {
        "id": "D",
        "text": "Allows them to validate the CMDB is configured to populate data for incomplete fields"
      }
    ],
    "answer": [
      "B"
    ],
    "explanation": "**[핵심 근거]**\nHealth Inclusion Rule은 조직이 CMDB Health Dashboard 계산에 포함할 CI와 해당 클래스를 필터링할 수 있도록 지원합니다. 이를 통해 관리 준비가 되지 않았거나 소유자가 없는 CI 데이터가 대시보드에 불필요하게 표시되는 것을 방지합니다.\n\n**[Exam Keywords]**\nCMDB, CMDB Health Dashboard, Health Inclusion Rule, CI"
  },
  {
    "id": 200,
    ...
    "explanation": "**[핵심 근거]**\n추천 필드(Recommended fields)에 대한 태스크 생성 기능은 CMDB Health Metrics 메뉴에서 설정합니다. Configuration > Health Preferences > Health Metrics로 이동한 뒤 Metric을 Recommended로 선택하고, Create Task를 활성화한 후 담당 그룹을 지정할 수 있습니다.\n\n**[Exam Keywords]**\nHealth Metrics"
  },
  {
    "id": 201,
    ...
    "options": [
      {
        "id": "A",
        "text": "700 server records are duplicates in the CMDB"
      },
      {
        "id": "B",
        "text": "800 server records are duplicates in the CMDB"
      },
      {
        "id": "C",
        "text": "100 server records are duplicates in the CMDB"
      },
      {
        "id": "D",
        "text": "700 Linux and 800 Windows server records are duplicates in the CMDB"
      }
    ],
    "answer": [
      "C"
    ],
    "explanation": "**[핵심 근거]**\n700/800 지표에서 앞의 700은 평가 대상인 전체 800개의 서버 CI 중 중복되지 않은 건강한(Healthy) CI의 수입니다. 따라서 전체 평가 대상에서 건강한 개수를 제외한 100개(800 - 700)가 중복 레코드가 됩니다.\n\n**[Exam Keywords]**\nCMDB, Correctness, CI"
  },
  {
    "id": 202,
    ...
    "options": [
      {
        "id": "A",
        "text": "800 server records exist in the CMDB"
      },
      {
        "id": "B",
        "text": "700 server records exist in the CMDB"
      },
      {
        "id": "C",
        "text": "100 server records exist in the CMDB"
      },
      {
        "id": "D",
        "text": "700 Linux and 800 Windows server records exist in the CMDB"
      }
    ],
    "answer": [
      "A"
    ],
    "explanation": "**[핵심 근거]**\n700/800 지표에서 분모인 800은 해당 중복 메트릭에서 평가 대상이 된 전체 서버 레코드의 범위(Scope)를 나타냅니다.\n\n**[Exam Keywords]**\nCMDB, Correctness"
  },
  {
    "id": 203,
    ...
    "explanation": "**[핵심 근거]**\nHealth Inclusion Rule은 CMDB 헬스 대시보드에 나타나는 개별 메트릭 계산에 포함될 CI 범위를 필터링하는 데 쓰입니다. Orphan 메트릭에 Server 클래스용 Health Inclusion Rule을 적용했기 때문에 해당 메트릭의 평가 대상 범위가 4200개로 줄어든 것입니다.\n\n**[Exam Keywords]**\nCMDB, Health Inclusion Rule, Correctness, CI"
  },
  {
    "id": 204,
    ...
    "options": [
      ...
      {
        "id": "C",
        "text": "Depends on the plugins enabled"
      },
      {
        "id": "D",
        "text": "Depends on the CIs discovered"
      }
    ],
    "answer": [
      "A"
    ],
    "explanation": "**[핵심 근거]**\n기본 제공 시스템(Base system) 환경에는 사전에 정의되거나 제공되는 고립 규칙(Orphan rules)이 기본적으로 존재하지 않습니다(0개).\n\n**[Exam Keywords]**\n0"
  },
  {
    "id": 205,
    ...
    "explanation": "**[핵심 근거]**\n기본 제공 시스템 환경에서 제공되는 기본 노후화 규칙(Staleness rule)의 유효 기준 기간은 60일로 설정되어 있습니다.\n\n**[Exam Keywords]**\n60 days"
  },
  {
    "id": 206,
    ...
    "options": [
      {
        "id": "A",
        "text": "Most recent discovery <last_discovered>"
      },
      {
        "id": "B",
        "text": "Created <sys_created_on>"
      ...
      {
        "id": "E",
        "text": "Discovery Source <discovery_source>"
      }
    ],
    "answer": [
      "C"
    ],
    "explanation": "**[핵심 근거]**\n노후화 메트릭(Staleness metric)은 레코드가 마지막으로 수정되거나 업데이트된 시간 정보가 담긴 Updated <sys_update_on> 타임스탬프 필드를 기준으로 계산됩니다.\n\n**[Exam Keywords]**\nUpdated <sys_update_on>"
  },
  {
    "id": 207,
    ...
    "explanation": "**[핵심 근거]**\n컴플라이언스 스코어카드(Compliance Scorecard) 결과 산출에는 오직 Desired State(원하는 상태) 감사와 Scripted Audit(스크립트 기반 감사) 두 가지 유형만 사용됩니다.\n\n**[Exam Keywords]**\nDesired State, Scripted Audit"
  },
  {
    "id": 208,
    ...
    "explanation": "**[핵심 근거]**\nDesired State 감사를 구성하는 요소로는 Certification Templates(인증 템플릿), Certification Filters(인증 필터), 그리고 Audit(감사 레코드)이 쓰입니다. Policies(정책)는 데이터 매니저(Data Manager)에서 사용되는 설정입니다.\n\n**[Exam Keywords]**\nCertification Templates, Certification Filters, Audit"
  },
  {
    "id": 209,
    ...
    "explanation": "**[핵심 근거]**\nCI 재분류(Reclassification) 프로세스 과정에서 클래스 계층 구조 하위로 내리는 Downgrade(강등)나 완전히 다른 계층으로 변경하는 Switch(전환)가 발생할 경우, 기존의 특정 속성과 데이터가 누락되거나 손실될 수 있습니다.\n\n**[Exam Keywords]**\nDowngrade, Switch"
  },
  {
    "id": 210,
    ...
    "options": [
      {
        "id": "A",
        "text": "glide.class.downgrade.enabled set to true and during discovery a Windows server is classified as a Linux server."
      },
      {
        "id": "B",
        "text": "glide.class.upgrade.enabled set to false and during discovery a Server is classified as a Windows server."
      }
    ],
    "answer": [
      "B"
    ],
    "explanation": "**[핵심 근거]**\n재분류 태스크(Reclassification task)는 해당 재분류 방향에 상응하는 시스템 속성(Property) 값이 false로 설정되어 자동 분류가 차단되었을 때 생성됩니다. 예를 들어 상위 개념인 Server가 Windows Server로 바뀐 것은 Upgrade 유형이며, `glide.class.upgrade.enabled` 값이 false이므로 시스템이 자동으로 처리하지 않고 사람이 검토하도록 재분류 태스크를 트리거합니다.\n\n**[Exam Keywords]**\nglide.class.upgrade.enabled, Discovery"
  },
  {
    "id": 211,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow Data Manager에서 공식 지원하는 정책 유형은 Retirement(은퇴), Certification(인증), Attestation(증명), Deletion(삭제), Archiving(보관)의 총 5가지입니다. 중복 제거(De-duplication)와 재분류(Reclassification)는 별도의 다른 전용 기능을 이용해 처리합니다.\n\n**[Exam Keywords]**\nRetirement, Certification, Attestation, Deletion, Archiving"
  },
  {
    "id": 212,
    ...
    "explanation": "**[핵심 근거]**\n조치 워크플로우(Remediation workflow)는 CMDB 조치 규칙(Remediation Rule)에 직접 정의되어 연결됩니다. 조치 태스크(Remediation Task)는 사용자가 수동 또는 자동화 프로세스를 통해 해당 조치 규칙과 워크플로우를 실행하도록 매개하는 주체입니다.\n\n**[Exam Keywords]**\nCMDB Remediation Rule"
  },
  {
    "id": 213,
    ...
    "options": [
      {
        "id": "A",
        "text": "Most Relationships"
      },
      {
        "id": "B",
        "text": "Most Recent Updated"
      },
      {
        "id": "C",
        "text": "Oldest Created"
    ...
    "explanation": "**[핵심 근거]**\nDuplicate CI Remediator 마법사에서 어떤 복제본을 대표 마스터 CI(Main CI)로 사용할지 제안할 때는 가장 많은 관계성 보유(Most Relationships), 가장 최근 수정(Most Recent Updated), 가장 먼저 생성됨(Oldest Created)의 세 가지 권장 기준이 반영됩니다.\n\n**[Exam Keywords]**\nMost Relationships, Most Recent Updated, Oldest Created"
  },
  {
    "id": 214,
    ...
    "options": [
      {
        "id": "A",
        "text": "The oldest CI, based on Creation Date is updated"
      },
      {
        "id": "B",
        "text": "The oldest CI, based on Last Updated is updated"
      },
      {
        "id": "C",
        "text": "The newest CI, based on Creation Date is updated"
      },
      {
        "id": "D",
        "text": "The newest CI, based on Last Updated is updated"
      }
    ],
    "answer": [
      "A"
    ],
    "explanation": "**[핵심 근거]**\nServiceNow Discovery를 진행하는 중 새로운 대상 장비를 발견했으나 CMDB 상에 이미 중복 데이터가 존재할 경우, 생성 날짜(Creation Date) 기준으로 가장 오래된 CI가 디스커버리에 의해 식별되어 최종 업데이트를 받습니다.\n\n**[Exam Keywords]**\nCMDB, Discovery, CI"
  },
  {
    "id": 215,
    ...
    "options": [
      ...
      {
        "id": "B",
        "text": "CI does not exist"
      },
      {
        "id": "C",
        "text": "CI has dependent CIs"
      },
      {
        "id": "D",
        "text": "CI does not have dependent CIs"
      }
    ],
    "answer": [
      "A"
    ],
    "explanation": "**[핵심 근거]**\nCI Attestation(CI 증명)은 해당 데이터가 가리키는 실제 IT 구성 항목 자산이 현실 세계에 실제로 존재함(CI exists)을 확인하고 보증하는 프로세스입니다.\n\n**[Exam Keywords]**\nCI exists"
  },
  {
    "id": 216,
    ...
    "explanation": "**[핵심 근거]**\n전체 Correctness 스코어카드는 그룹 내 CI가 하위의 '모든' 메트릭(중복 없음, 고립 없음, 노후화 없음)을 결함 없이 통과해야 정상 건수로 판단합니다. 10대 중 Server 1, 2, 3 총 3대가 각기 다른 메트릭에 결함이 발생했으므로 모든 메트릭을 통과한 정상 CI는 7대뿐이어서 최종 70%가 기록됩니다.\n\n**[Exam Keywords]**\n70%"
  },
  {
    "id": 217,
    ...
    "explanation": "**[핵심 근거]**\n질문은 전체 Correctness가 아닌 개별 하위 지표인 '중복 없음(non-duplicate)' 메트릭만의 비율을 묻고 있습니다. 10대 중 오직 Server 1과 Server 2만 중복 메트릭에서 실패로 분류되었으며(Server 3, 4는 다른 메트릭 결함), 나머지 8대는 중복 검사를 무사히 통과했으므로 80%가 정답입니다.\n\n**[Exam Keywords]**\n80%"
  },
  {
    "id": 218,
    ...
    "options": [
      {
        "id": "A",
        "text": "Non-Duplicate CIs 12/20"
      },
      {
        "id": "B",
        "text": "Non-Duplicate CIs 10/20%"
      },
      {
        "id": "C",
        "text": "Non-Duplicate CIs 16/20"
      },
      {
        "id": "D",
        "text": "Non-Duplicate CIs 15/20"
      }
    ],
    "answer": [
      "A"
    ],
    "explanation": "**[핵심 근거]**\n20개 중 12개의 레코드가 중복되지 않은(Non-Duplicate) 정상 데이터로 판정됩니다. Server 1~4는 중복 레코드가 존재하고 discovery source 필드가 없어 총 8개의 레코드가 중복 결함 항목으로 잡힙니다. 반면 Server 5는 중복 관계임에도 양쪽 모두에 discovery source 필드가 유효하게 채워져 있으므로 IRE(식별 및 조정 엔진)에 의해 유효하게 취급되어 중복 플래그에서 제외됩니다. 따라서 정상 처리된 결과물은 12/20이 됩니다.\n\n**[Exam Keywords]**\nNon-Duplicate CIs 12/20"
  },
  {
    "id": 219,
    ...
    "options": [
      ...
      {
        "id": "D",
        "text": "Duplicate CI Remediator"
      }
    ],
    "answer": [
      "A"
    ],
    "explanation": "**[핵심 근거]**\nNow Assist for CMDB의 AI 기반 자동화 중복 CI 관리 기능은 감지 및 조치 워크플로우를 고도화하기 위해 De-Duplication Templates(중복 제거 템플릿) 기술을 활용합니다.\n\n**[Exam Keywords]**\nDe-Duplication Templates"
  },
  {
    "id": 220,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow University 교육 과정 매칭 문제로, CMDB(a), Class(b), Attributes(c), Foundational Data(d)의 공식 정의를 올바르게 설명하고 있습니다.\n\n**[Exam Keywords]**\n1-a, 2-b, 3-c, 4-d"
  },
  {
    "id": 221,
    ...
    "explanation": "**[핵심 근거]**\n각 CI 클래스는 상속 관계에 따라 특정 테이블에 저장됩니다. cmdb_ci(a)는 최상위 베이스 테이블이며, cmdb_ci_hardware(b)는 하드웨어 클래스, cmdb_ci_server(c)는 서버 클래스, cmdb_ci_service_auto(d)는 애플리케이션 서비스 테이블입니다.\n\n**[Exam Keywords]**\n1-a, 2-b, 3-c, 4-d"
  },
  {
    "id": 222,
    ...
    "explanation": "**[핵심 근거]**\nScanning은 Shazzam 프로브를 활용해 활성 포트를 검사하며(b), Classification은 OS와 기기 유형을 분류하고(c), Identification은 IRE 식별 규칙으로 중복을 방지하며 기기 생성 여부를 결정하고(d), Exploration은 상세 속성 및 관계를 최종 탐색(a)합니다.\n\n**[Exam Keywords]**\n1-b, 2-c, 3-d, 4-a"
  },
  {
    "id": 223,
    ...
    "explanation": "**[핵심 근거]**\nFoundation은 Location/User(a) 등 기본 프레임이며, Design & Planning은 아키텍처 관점의 Business Application(b)을 다룹니다. Service Delivery는 운영 환경의 Application Service(c)를 포함하며, Service Consumption은 고객이 구독/신청하는 Business Service와 Service Offering(d)을 매핑합니다.\n\n**[Exam Keywords]**\n1-a, 2-b, 3-c, 4-d"
  },
  {
    "id": 224,
    ...
    "explanation": "**[핵심 근거]**\nRecommended 지표는 권장 필드 누락(b), Desired State는 정책 준수 감사(c), Staleness는 갱신되지 않은 오래된 CI(a), Orphan은 관계선이 끊긴 고아 CI(d)를 분류합니다.\n\n**[Exam Keywords]**\n1-b, 2-c, 3-a, 4-d"
  },
  {
    "id": 225,
    ...
    "explanation": "**[핵심 근거]**\nUpgrade는 더 구체적인 자식 클래스로 이동하므로 정보 손실이 없고(b), Downgrade는 상위 부모로 가면서 자식만의 세부 필드가 누락될 위험이 있으며(c), Switch는 다른 자식 분기로 변경 시 필드가 유실될 위험이 발생(a)합니다.\n\n**[Exam Keywords]**\n1-b, 2-c, 3-a"
  },
  {
    "id": 226,
    ...
    "explanation": "**[핵심 근거]**\nPrincipal Class는 검색용 필터 목록(a)을 설정하고, Suggested Relationships는 관계 종류 가이드(b)를 제시하며, Identification Rules는 중복 식별 기준(c), Reconciliation Rules는 소스 신뢰도 제어(d)를 설정합니다.\n\n**[Exam Keywords]**\n1-a, 2-b, 3-c, 4-d"
  },
  {
    "id": 227,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow CMDB/CSDM 운영 체계의 역할 정의 매칭으로, 기술 인프라 담당(CMDB Admin - a), 프로세스 및 정책 총괄(Configuration Manager - b), 실무 데이터 분석 및 복구 태스크 수행(CI Analyst - c), 서비스의 비즈니스 가치 및 품질 보증(Service Owner - d)을 올바르게 설명합니다.\n\n**[Exam Keywords]**\n1-a, 2-b, 3-c, 4-d"
  },
  {
    "id": 228,
    ...
    "explanation": "**[핵심 근거]**\n각 탐색 대상별 프로토콜 매치 문제로, Windows는 WMI/WinRM(a), Linux/Unix는 SSH(b), 네트워크 장비는 SNMP(c), 가상화 서버는 VMware API(d)를 사용합니다.\n\n**[Exam Keywords]**\n1-a, 2-b, 3-c, 4-d"
  },
  {
    "id": 229,
    ...
    "explanation": "**[핵심 근거]**\nMID Server 통신 구조 매칭으로, MID Server는 아웃바운드 443 포트로 통신하며(a), ecc_queue 테이블이 완충 지대 역할을 하고(b), Output 큐는 인스턴스에서 MID 서버로 향하는 명령 지시(c), Input 큐는 수집된 결과의 보고(d)를 의미합니다.\n\n**[Exam Keywords]**\n1-a, 2-b, 3-c, 4-d"
  },
  {
    "id": 230,
    ...
    "explanation": "**[핵심 근거]**\nServiceNow University 교육 과정 및 자격증 시험에서 출제되는 데이터 수집 방법별 표준 문장 정의를 짝짓는 문제입니다. Discovery는 IP 대역 기반 수평적 에이전트리스 스캔(a), Service Mapping은 비즈니스 관점 하향식 종속성 매핑(b), Service Graph Connectors는 IRE가 내장된 서드파티 연동 표준 앱(c), Import Sets는 스테이징 테이블과 Transform Map을 활용하는 수동 데이터 로더(d)입니다.\n\n**[Exam Keywords]**\n1-a, 2-b, 3-c, 4-d"
  }
];
