window.CAD_QUESTIONS = [
  ...
  },
  {
    ...
    "options": [
      ...
      {
        "text": "A Transform Map that loads records into the module’s target table",
        "id": "B"
      },
      {
        "text": "A Scheduled Job that refreshes application records periodically",
        "id": "C"
      },
      {
        "text": "A UI Policy that controls fields after a form has opened",
        "id": "D"
      }
    ],
    "answer": [
  ...
  },
  {
    ...
    "options": [
      ...
      {
        "text": "Render an import-review form without mapping source data",
        "id": "B"
      },
      {
        "text": "Grant application roles to users referenced in source rows",
        "id": "C"
      },
      {
        "text": "Run a client script for every server-side transformation row",
        "id": "D"
      }
    ],
    "answer": [
  ...
  },
  {
    ...
    "options": [
      ...
      {
        "text": "Give every user unrestricted development access across all application scopes",
        "id": "B"
      },
      {
        "text": "Replace source control and application review with direct production editing",
        "id": "C"
      },
      {
        "text": "Disable application-scope boundaries for all delegated developers",
        "id": "D"
      }
    ],
    "answer": [
...
[34,'What is the main purpose of application scope?','Isolate application artifacts and control cross-scope access','Replace every record and field ACL used by the application','Create production clones containing only the scoped application','Store application authentication credentials inside the scope record','Application Scope는 이름 충돌을 방지하고 애플리케이션 파일과 데이터의 경계를 제어합니다.'],
...
[44,'Why test security with impersonation or a representative non-admin user?','Admin privileges can mask missing or incorrect access controls','It permanently rotates the impersonated user’s authentication credential','It automatically creates Update Sets containing discovered ACL differences','It compiles all application scripts using the impersonated user’s roles','관리자는 일반 사용자의 ACL 결과와 다른 권한을 가지므로 실제 역할로 검증해야 합니다.'],
...
[48,'What is the purpose of a data policy?','Enforce data requirements across multiple input channels, including imports and web services','Change the visual styling of fields only when a record is opened in a browser form','Package application files for promotion without enforcing record data requirements','Select a MID Server for imports performed through a web-service endpoint','Data Policy는 UI뿐 아니라 Import·API 등 다양한 경로의 데이터 요구사항을 적용할 수 있습니다.'],
...
[54,'What does coalesce do in a Transform Map?','Identify an existing target record for update instead of always inserting','Combine two Update Sets into one source-data transformation','Encrypt every imported value before the target table is selected','Make every target field mandatory for all transformed rows','Coalesce 필드는 기존 대상 레코드를 찾아 Insert와 Update를 구분합니다.'],
[55,'What is a Flow Designer subflow useful for?','Reusable automation logic that can be called from multiple flows','A client-side form API used to manipulate fields during flow execution','A CMDB identification rule that reconciles records created by flows','A database index created for every table referenced by the automation','Subflow는 입력·출력을 가진 재사용 가능한 자동화 단위를 제공합니다.'],
...
[60,'Which practice best supports production-ready custom applications?','Automated tests, least privilege, documented configuration, peer review, and controlled deployment','Develop exclusively with administrator access directly in production','Turn off application logging and auditing before deployment','Embed every environment-specific value permanently in application scripts','테스트·최소 권한·문서화·리뷰·통제된 배포가 안전하고 유지 가능한 애플리케이션의 기반입니다.']
...
(function(qs){var rows=[
[101,'What determines which users can see an application menu or module in the All menu?','The roles configured on the application menu and module','The active Update Set containing the navigation records','The label configured on the module’s target table','The browser language selected in the user session','Application navigation artifacts can restrict visibility with role requirements.','User Interface'],
...
[104,'Why create a separate form view for a specialist role?','Present a task-specific arrangement of fields without changing the underlying record','Create a duplicate table containing records for only that specialist role','Bypass field ACL evaluation whenever the specialist opens the form','Store a second copy of each record using the specialist layout','Views change presentation; they do not duplicate data or replace security.','User Interface'],
...
[111,'Which statement about g_user.hasRole() is important when testing with administrators?','An administrator may satisfy role checks broadly, so impersonation testing is important','It is a server-only method that can execute exclusively inside Business Rules','It returns a GlideRecord containing all roles assigned to the current user','It permanently modifies the roles of the user being evaluated','Administrator behavior can mask role-specific client behavior; test with representative users.','Security'],
...
[113,'A developer wants environment-specific configuration without hard-coding it in scripts. What should be used?','A system property read with gs.getProperty()','A duplicated Client Script constant maintained separately on every form','The application label changed independently in every environment','An authentication credential embedded directly in a Script Include','Properties externalize configurable values from application logic.','Application Design'],
...
[121,'What is a Data Pill in Flow Designer?','A selectable runtime value produced by a trigger or previous step','A stored authentication value automatically exposed to every flow action','A database index created whenever a flow reads a record','A source-control commit containing the flow’s latest execution data','Data Pills pass typed runtime data between flow steps.','Automation & Integration'],
...
[124,'What should be checked first when a flow produced unexpected output?','The flow execution details and step data','The user’s web navigation activity before starting the flow','The description of the active Update Set containing unrelated changes','The form layout used to display the record after execution','Execution details show inputs, outputs, status, and errors for each step.','Automation & Integration'],
[125,'A Flow Designer inline script supplies a Date/Time input. What must it return?','A value compatible with the expected Date/Time data type','Any string value regardless of its format or the target input type','A UI Action object representing the form where the flow was started','No value because Flow Designer always derives Date/Time inputs automatically','Inline scripts must return the data type required by the target input.','Automation & Integration'],
...
[129,'What is a Scripted REST API resource?','A defined inbound endpoint with server logic for an HTTP method and path','An outbound REST credential used by RESTMessageV2 integrations','A Client Script callback that executes after a form request','An Update Set source that retrieves application files from another instance','Resources define the methods and paths served by a Scripted REST API.','Automation & Integration'],
[130,'What should protect a Scripted REST API that returns application data?','Authentication plus appropriate ACL or scripted authorization checks','A hidden form field that is never evaluated by the inbound API request','A UI Policy applied only when users access the corresponding browser form','An inactive Update Set containing the API resource definition','Inbound APIs require explicit authorization and data-access controls.','Security'],
...
[166,'A scoped application calls a protected API in another scope and the runtime access is set to tracking. What may the platform create?','A cross-scope privilege record requiring an allow or deny decision','A user role that automatically grants access to every protected API','A duplicate application containing a copy of the called API','A Transform Map that converts the request into an allowed operation','Runtime access tracking records attempted cross-scope use for governance.','Application Design'],
...
[172,'Which RESTMessageV2 method assigns a named path or query parameter defined on the REST message record?','setStringParameterNoEscape() or the appropriate parameter setter','addAggregate() after grouping the outbound request parameters','setAbortAction() before executing the configured REST method','showFieldMsg() with the parameter name and replacement value','REST parameter setters substitute values into configured request variables.','Automation & Integration'],
...
[200,'What is a formatter used for on a form?','Display specialized non-field content or functionality in the layout','Define a database index for fields displayed in the form layout','Grant a role whenever a user opens a particular form view','Transform imported rows before the target record form is displayed','Formatters add platform components such as activity or process content to forms.','User Interface'],
[201,'A developer considers a UI Macro for a new experience. What should be checked before using it?','Whether a supported modern component or declarative option better fits the target interface','Whether the target table uses a coalesce field intended for Import Sets','Whether the current Update Set has the name Default','Whether all application users have elevated administrative privileges','UI technology support varies by interface, so choose the supported extension mechanism.','User Interface'],
...
[213,'A user may edit most fields but not the approval field. What should enforce this?','A field-level Write ACL on the approval field','A hidden navigation module that does not govern API or list edits','A separate form section containing the approval field','A report filter that omits approval values from one report','Field Write ACLs protect updates through forms, APIs, imports, and scripts that enforce security.','Security'],
[214,'A Before Query Business Rule restricts rows for a class of users. What is the key design concern?','Test all access paths and avoid using it as a careless substitute for clear ACL design','It executes only after every matching record has already been displayed','It modifies the table schema each time a restricted query runs','It is client-side logic evaluated only on browser forms','Query rules alter returned data broadly and require careful security and performance testing.','Security'],
[215,'What should a custom application role represent?','A stable job capability required to use application features','One named individual rather than a reusable work capability','The lifecycle state of the Update Set containing the application','One browser session created when a user opens the application','Capability-based roles are reusable and can be assigned through groups.','Security'],
[216,'A role includes another role. What is the intended benefit?','Build reusable permission hierarchies from smaller capabilities','Copy users assigned to the contained role into another instance','Merge application scopes whenever their roles share a name','Skip ACL evaluation for users holding the broader role','Role containment lets broader personas inherit narrower capabilities.','Security'],
[217,'A table must be accessible through web services only when explicitly allowed. Which setting is relevant in addition to ACLs?','The table Application Access web-service permission','The form view used when a browser user opens the table record','The navigation-module order assigned to the application menu','The Update Source URL used to retrieve remote Update Sets','Table application access can restrict web-service operations, while ACLs still authorize users.','Security'],
...
[230,'A successful REST response body contains JSON. What should server-side code do before using nested properties?','Parse the JSON and validate the expected structure','Execute the response body directly as trusted server-side JavaScript','Store the entire response as an application authentication credential','Assume every nested property exists whenever the HTTP status is successful','Controlled parsing and validation prevent malformed responses from breaking processing.','Working with External Data'],
...
[234,'A transform row cannot be accepted because required identity data is missing. What should the transform logic do?','Reject or ignore the row with traceable error information','Insert an arbitrary target record without a stable identity value','Disable every coalesce field for all remaining rows in the Import Set','Commit the active Update Set before continuing the transformation','Invalid rows should be handled explicitly and remain diagnosable.','Working with External Data'],
