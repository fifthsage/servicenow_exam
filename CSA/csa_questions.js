window.CSA_QUESTIONS = [
  ...
  },
  {
    ...
    "explanation": "**Service Catalog**는 사용자가 표준화된 화면에서 서비스를 요청하거나 필요한 업무 레코드를 생성할 수 있게 해주는 셀프서비스 영역입니다.\n*   **정답 이유 (A)**: Catalog Items는 사용자가 요청할 수 있는 개별 서비스/상품이고, Record Producers는 카탈로그 화면을 통해 Incident 같은 task-based record를 생성하며, Order Guides는 여러 Catalog Item을 하나의 요청 흐름으로 묶습니다.\n*   **오답 구분**: Item Variables와 Workflows는 카탈로그 항목을 구성하는 내부 요소이지, 사용자가 요청을 시작할 때 고르는 대표 구성 요소로 보기 어렵습니다.\n*   **시험 포인트**: 최신 화면명이 바뀌더라도 Catalog Item, Record Producer, Order Guide의 역할 구분은 CSA에서 계속 중요한 기본 개념입니다."
  },
  {
    "id": 27,
  ...
  },
  {
    ...
    "explanation": "Classic 플랫폼 용어에서 ServiceNow UI는 크게 **Banner Frame**, **Application Navigator**, **Content Frame**으로 설명됩니다.\n*   **정답 이유 (B, D, F)**: Banner Frame은 상단 제어 영역, Application Navigator는 앱과 모듈을 찾는 탐색 영역, Content Frame은 리스트/폼/보고서가 표시되는 작업 영역입니다.\n*   **오답 구분**: Banner Navigator, Application Frame, Content Menu는 이 3대 UI 영역을 지칭하는 표준 명칭이 아닙니다.\n*   **시험 포인트**: 이 문항은 최신 화면 디자인 자체보다 탐색 영역과 작업 영역을 구분하는 기본 UI 구조를 묻습니다. Classic 용어임을 전제로 학습하면 혼란이 줄어듭니다."
  },
  {
    "id": 65,
  ...
  },
  {
    ...
    "explanation": "폼에서 현재 레코드에 대한 저장, 구성, 템플릿 적용 같은 작업은 **Additional Actions / Context Menu**에서 접근합니다.\n*   **정답 이유 (B)**: 이 메뉴는 현재 폼/레코드에 대한 작업을 모아 제공하는 기능 영역입니다.\n*   **오답 구분**: Application Navigator는 앱과 모듈을 찾는 탐색 영역이고, Form Designer는 폼을 설계하는 관리자 도구입니다. Navigation menu는 기능을 너무 포괄적으로 표현한 보기입니다.\n*   **시험 포인트**: 아이콘 모양을 외우기보다, 폼의 레코드 수준 작업은 Additional Actions 또는 Context Menu에서 찾는다는 기능 중심으로 기억하는 것이 좋습니다."
  },
  {
    "id": 97,
  ...
  },
  {
    ...
    "explanation": "Priority처럼 드롭다운 선택값을 바꾸려면 필드의 **Choices**를 구성해야 합니다.\n*   **정답 이유 (D)**: `Configure Choices`는 선택 목록에 표시되는 값과 레이블을 관리하는 작업입니다. Task 수준에서 Priority choice를 조정하면 이를 확장한 Incident, Problem, Change 등 하위 task 테이블에도 일관된 용어를 적용할 수 있습니다.\n*   **오답 구분**: List Layout이나 Form Layout은 화면 배치 변경이고, Dictionary는 필드 속성 자체를 관리하는 영역입니다. UI Policy는 조건에 따라 필드 표시/필수/읽기 전용 상태를 제어할 때 사용합니다.\n*   **시험 포인트**: 필드 제목을 바꾸는 것과 choice value/label을 바꾸는 것은 다릅니다. 드롭다운 값 표현을 바꾸는 문제는 Choices 관점으로 판단해야 합니다."
  },
  {
    "id": 112,
  ...
  },
  {
    ...
    "explanation": "ServiceNow의 **Access Control (ACL)** 규칙은 사용자가 특정 데이터에 대해 어떤 작업을 수행할 수 있는지 평가합니다.\n*   **보안 대상**: ACL은 테이블(Table), 레코드/행(Row), 필드/열(Column) 수준에서 접근을 제어할 수 있습니다.\n*   **평가 요소**: 역할(Role), 조건(Condition), 스크립트 등을 조합하여 read, write, create, delete 같은 작업(Operation)을 허용할지 판단합니다.\n*   **오답 구분**: 역할, 그룹, 사용자는 ACL이 평가에 사용할 수 있는 기준이지, ACL이 보호하는 데이터베이스 객체 수준 자체는 아닙니다."
  },
  {
    "id": 162,
  ...
  },
  {
    ...
    "explanation": "ServiceNow는 날짜와 시간을 내부적으로 표준화해 저장하고, 사용자에게 표시할 때는 사용자의 시간대 설정을 기준으로 변환합니다.\n*   **정답 이유 (B)**: 특정 사용자의 Incident 목록에서 생성 시간이 지역과 맞지 않게 보이면, 해당 사용자의 profile/preferences에서 **time zone**을 올바르게 지정하도록 안내하는 것이 표준적인 조치입니다.\n*   **오답 구분**: 브라우저 캐시, 브라우저 종류, 컴퓨터의 로컬 시간대, 인스턴스 전체 시스템 시간대 변경은 특정 사용자 한 명의 표시 시간 문제를 해결하는 적절한 1차 조치가 아닙니다.\n*   **시험 포인트**: 화면의 gear icon 같은 경로보다, 사용자별 시간 표시는 사용자 시간대 설정을 따른다는 원리를 기억하는 것이 중요합니다."
  },
  {
    "id": 165,
  ...
  },
  {
    ...
    "explanation": "Classic 플랫폼 브랜딩의 배너 이미지, 텍스트, 색상 같은 전역 룩앤필 설정은 **System Properties > Basic Configuration UI16**에서 관리합니다.\n*   **정답 이유 (C)**: 이 모듈은 기본 플랫폼 UI의 브랜딩 설정을 조정하는 영역입니다.\n*   **오답 구분**: Service Portal > Portals > Branding은 포털 화면 전용 브랜딩 설정이며, 플랫폼 기본 UI 브랜딩과는 범위가 다릅니다.\n*   **시험 포인트**: UI16은 Classic UI 용어이지만, 시험 의도는 '플랫폼 전역 브랜딩'과 '포털 브랜딩'의 차이를 구분하는 데 있습니다."
  },
  {
    "id": 186,
  ...
  },
  {
    ...
    "explanation": "관련 리스트(Related List)의 표시 컬럼을 개인화하려면 리스트 개인화 컨트롤을 사용합니다.\n*   **정답 이유 (D)**: List personalization controls는 현재 사용자에게 보이는 컬럼의 종류와 순서를 조정하는 개인 설정입니다.\n*   **오답 구분**: Magnifier는 검색/필터 보조 기능이고, Context Menu는 레코드나 리스트 작업 메뉴이며, Pencil은 일반적으로 즐겨찾기 편집 같은 별도 UI 동작에 사용됩니다.\n*   **시험 포인트**: 개인화(Personalization)는 현재 사용자에게만 영향을 주며, 모든 사용자에게 적용되는 기본 리스트 구성을 바꾸려면 관리자가 `Configure > List Layout`을 사용합니다."
  },
  {
    "id": 297,
    ...
    "explanation": "ServiceNow의 메뉴 가시성과 작업 가능 범위는 사용자의 **역할(Role)**에 의해 제어됩니다.\n*   **정답 이유 (B, D)**: IT 실무자가 task/incident 같은 작업 레코드를 처리하려면 일반적으로 **itil** 역할이 필요합니다. 사용자가 itil 역할을 직접 갖고 있지 않거나, itil 역할이 포함된 그룹에 속하지 않으면 필요한 IT task modules가 보이지 않을 수 있습니다.\n*   **오답 구분**: LDAP 인증 실패라면 로그인 자체가 어렵고, 관리자 승인이나 \"로그인이 제대로 되지 않음\"은 ServiceNow의 표준 역할 기반 메뉴 가시성 원인을 설명하기에는 모호합니다.\n*   **시험 포인트**: 메뉴가 안 보이는 문제는 먼저 역할과 그룹 멤버십을 확인합니다."
  },
  {
    "id": 298,
  ...
  },
  {
    ...
    "explanation": "ServiceNow 리스트 뷰에서 컬럼 구성(추가, 제거, 순서)을 시스템 수준에서 수정하려면 **Configure > List Layout**을 사용합니다.\n*   **정답 이유 (A)**: 컬럼 헤더의 컨텍스트 메뉴에서 Configure > List Layout을 선택하면 해당 리스트의 기본 컬럼 구성을 관리할 수 있습니다.\n*   **오답 구분**: 개인화 컨트롤은 현재 사용자에게만 적용되는 User Preference에 가깝고, 시스템 기본 레이아웃 변경과는 범위가 다릅니다.\n*   **시험 포인트**: `personalize list columns`는 개인 설정, `Configure > List Layout`은 관리자 관점의 기본 레이아웃 변경으로 구분해야 합니다."
  },
  {
    "id": 362,
